...
describe("vector", function()
	...
	it("set_copy()", function()
		local v = vector.zero()
		local w = vector.new(1, 2, 3)
		vector.set_copy(v, w)
		assert.same(v, w)
		v = vector.zero()
		v:set_copy(w)
	...
	it("set_apply()", function()
		local i = 0
		local f = function(x)
			i = i + 1
			return x + i
		end
		local original = vector.new(0.1, 0.9, -0.5)
		local a = vector.zero()
		vector.set_apply(a, original, math.ceil)
		assert.equal(vector.new(1, 1, 0), a)
		a:set_apply(original, math.ceil)
		assert.equal(vector.new(1, 1, 0), a)
		vector.set_apply(a, original, math.abs)
		assert.equal(vector.new(0.1, 0.9, 0.5), a)
		a:set_apply(original, math.abs)
		assert.equal(vector.new(0.1, 0.9, 0.5), a)
		vector.set_apply(a, original, f)
		assert.equal(vector.new(1.1, 2.9, 2.5), a)
		a:set_apply(original, f)
		assert.equal(vector.new(4.1, 5.9, 5.5), a)
	end)
	...
	it("set_add()", function()
		...
		local b = vector.new(1, 4, 3)
		local c = vector.new(2, 6, 6)
		local a_modified = vector.zero()
		vector.set_add(a_modified, a, {x = 1, y = 4, z = 3})
		assert.equal(c, a_modified)
		a_modified = vector.zero()
		vector.set_add(a_modified, a, b)
		assert.equal(c, a_modified)
		a_modified = vector.zero()
		a_modified:set_add(a, b)
		assert.equal(c, a_modified)
	end)

	it("set_subtract()", function()
		local a = vector.new(1, 2, 3)
		local b = vector.new(2, 4, 3)
		local c = vector.new(-1, -2, 0)
		local a_modified = vector.zero()
		vector.set_subtract(a_modified, a, {x = 2, y = 4, z = 3})
		assert.equal(c, a_modified)
		a_modified = vector.zero()
		vector.set_subtract(a_modified, a, b)
		assert.equal(c, a_modified)
		a_modified = vector.zero()
		a_modified:set_subtract(a, b)
		assert.equal(c, a_modified)
	end)

	it("set_multiply()", function()
		local a = vector.new(1, 2, 3)
		local s = 2
		local b = vector.new(2, 4, 6)
		local a_modified = vector.zero()
		vector.set_multiply(a_modified, a, s)
		assert.equal(b, a_modified)
		a_modified = vector.zero()
		a_modified:set_multiply(a, s)
		assert.equal(b, a_modified)
	end)

	it("set_divide()", function()
		local a = vector.new(1, 2, 3)
		local s = 2
		local b = vector.new(0.5, 1, 1.5)
		local a_modified = vector.zero()
		vector.set_divide(a_modified, a, s)
		assert.equal(b, a_modified)
		a_modified = vector.zero()
		a_modified:set_divide(a, s)
		assert.equal(b, a_modified)
	end)
