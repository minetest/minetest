...
describe("vector", function()
	...
	it("indexes", function()
		local some_vector = vector(24, 42, 13)
	...
	it("direction()", function()
		...
		local b = vector(1, 42, 0)
		assert.equal(vector(0, 1, 0), vector.direction(a, b))
		assert.equal(vector(0, 1, 0), a:direction(b))
	end)

	it("distance()", function()
		local a = vector(1, 0, 0)
		local b = vector(3, 42, 9)
	...
	it("normalize()", function()
		local a = vector(0, 0, -23)
		assert.equal(vector(0, 0, -1), vector.normalize(a))
		assert.equal(vector(0, 0, -1), a:normalize())
		assert.equal(vector.new(), vector.normalize(vector.new()))
	end)

	it("floor()", function()
		local a = vector(0.1, 0.9, -0.5)
		assert.equal(vector(0, 0, -1), vector.floor(a))
		assert.equal(vector(0, 0, -1), a:floor())
	end)

	it("round()", function()
		local a = vector(0.1, 0.9, -0.5)
		assert.equal(vector(0, 1, -1), vector.round(a))
		assert.equal(vector(0, 1, -1), a:round())
	end)
	...
	it("apply()", function()
		...
		local a = vector(0.1, 0.9, -0.5)
		assert.equal(vector(1, 1, 0), vector.apply(a, math.ceil))
		assert.equal(vector(1, 1, 0), a:apply(math.ceil))
		assert.equal(vector(0.1, 0.9, 0.5), vector.apply(a, math.abs))
		assert.equal(vector(0.1, 0.9, 0.5), a:apply(math.abs))
		assert.equal(vector(1.1, 2.9, 2.5), vector.apply(a, f))
		assert.equal(vector(4.1, 5.9, 5.5), a:apply(f))
	end)
	...
	it("combine()", function()
		...
		assert.equal(vector(3, 2, 3), vector.combine(a, b, math.max))
		assert.equal(vector(1, 2, 1), vector.combine(a, b, math.min))
	end)
	...
	it("equals()", function()
		...
		assertE({x = -1, y = 0, z = 1}, vector(-1, 0, 1))
		local a = {x = 2, y = 4, z = -10}
		assertE(a, a)
		assertNE({x = -1, y = 0, z = 1}, a)

		assert.equal(vector(1, 2, 3), vector(1, 2, 3))
		assert.is_true(vector(1, 2, 3):equals(vector(1, 2, 3)))
		assert.not_equal(vector(1, 2, 3), vector(1, 2, 4))
		assert.is_true(vector(1, 2, 3) == vector(1, 2, 3))
		assert.is_false(vector(1, 2, 3) == vector(1, 3, 3))
	end)
	...
	it("sort()", function()
		local a = vector(1, 2, 3)
		local b = vector(0.5, 232, -2)
		local sorted = {vector(0.5, 2, -2), vector(1, 232, 3)}
		assert.same(sorted, {vector.sort(a, b)})
		assert.same(sorted, {a:sort(b)})
	end)

	it("angle()", function()
		assert.equal(math.pi, vector.angle(vector(-1, -2, -3), vector(1, 2, 3)))
		assert.equal(math.pi/2, vector(0, 1, 0):angle(vector(1, 0, 0)))
	end)

	it("dot()", function()
		assert.equal(-14, vector.dot(vector(-1, -2, -3), vector(1, 2, 3)))
		assert.equal(0, vector.new():dot(vector(1, 2, 3)))
	end)

	it("cross()", function()
		local a = vector(-1, -2, 0)
		local b = vector(1, 2, 3)
		assert.equal(vector(-6, 3, 0), vector.cross(a, b))
		assert.equal(vector(-6, 3, 0), a:cross(b))
	end)

	it("offset()", function()
		assert.same({x = 41, y = 52, z = 63}, vector.offset(vector(1, 2, 3), 40, 50, 60))
		assert.equal(vector(41, 52, 63), vector.offset(vector(1, 2, 3), 40, 50, 60))
		assert.equal(vector(41, 52, 63), vector(1, 2, 3):offset(40, 50, 60))
	end)
	...
	it("add()", function()
		local a = vector(1, 2, 3)
		local b = vector(1, 4, 3)
		local c = vector(2, 6, 6)
	...
	it("subtract()", function()
		local a = vector(1, 2, 3)
		local b = vector(2, 4, 3)
		local c = vector(-1, -2, 0)
	...
	it("multiply()", function()
		local a = vector(1, 2, 3)
		local b = vector(2, 4, 3)
		local c = vector(2, 8, 9)
		local s = 2
		local d = vector(2, 4, 6)
	...
	it("divide()", function()
		local a = vector(1, 2, 3)
		local b = vector(2, 4, 3)
		local c = vector(0.5, 0.5, 1)
		local s = 2
		local d = vector(0.5, 1, 1.5)
	...
	it("dir_to_rotation()", function()
		...
		local function forward_at_rot(rot)
			return vector.rotate(vector(0, 0, 1), rot)
		end
		local function up_at_rot(rot)
			return vector.rotate(vector(0, 1, 0), rot)
		end
		...
		for i = 1, 1000 do
			local rand_vec = vector(math.random(), math.random(), math.random())
