...
describe("vector", function()
	describe("new()", function()
		it("constructs", function()
			-- Note: vector.new actually returns tables of the format {num, num, num},
			-- but next() was overridden, which seems to be used by same
			assert.same({x = 0, y = 0, z = 0}, vector.new())
			assert.same({x = 1, y = 2, z = 3}, vector.new(1, 2, 3))
			assert.same({x = 3, y = 2, z = 1}, vector.new({x = 3, y = 2, z = 1}))

			assert.is_true(vector.is_vector(vector.new()))
			assert.is_true(vector.is_vector(vector.new(1, 2, 3)))
			assert.is_true(vector.is_vector(vector.new({x = 3, y = 2, z = 1})))
	...
	it("distance()", function()
		...
		assert.is_true(math.abs(43 - vector.distance(a, b)) < 0.000000000001)
		assert.is_true(math.abs(43 - a:distance(b)) < 0.000000000001)
	...
	it("round()", function()
		local a = vector.new(0.1, 0.9, -0.5)
		assert.equal(vector.new(0, 1, 0), vector.round(a))
		assert.equal(vector.new(0, 1, 0), a:round())
	end)
	...
	it("metatable is same", function()
		...
		assert.equal(true, vector.is_vector(a))
		assert.equal(true, vector.is_vector(b))
	...
	it("is_vector()", function()
		...
		assert.is_false(vector.is_vector(nil))
		assert.is_false(vector.is_vector(1))
		assert.is_false(vector.is_vector(true))
		assert.is_false(vector.is_vector("foo"))
		assert.is_false(vector.is_vector(some_table1))
		assert.is_false(vector.is_vector(some_table2))
		assert.is_false(vector.is_vector(some_table3))
		assert.is_false(vector.is_vector(some_table4))
		assert.is_false(vector.is_vector(old))
		assert.is_true(vector.is_vector(real))
		assert.is_true(real:is_vector())
	end)
