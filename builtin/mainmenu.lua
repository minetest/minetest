...
function update_menu()
	...
	if gamedata.errormessage ~= nil then
		formspec = formspec ..
			...
			";]"..
			"button[4.5,4.2;3,0.5;btn_error_confirm;" .. gettext("Ok") .. "]"
	else
...
function tabbuilder.dialog_create_world()
	...
	local retval = 
		"label[2,0;" .. gettext("World name") .. "]"..
		"label[2,1;" .. gettext("Mapgen") .. "]"..
		"field[4.5,0.4;6,0.5;te_world_name;;]" ..
		"label[2,2;" .. gettext("Game") .. "]"..
		"button[5,4.5;2.6,0.5;world_create_confirm;" .. gettext("Create") .. "]" ..
		"button[7.5,4.5;2.8,0.5;world_create_cancel;" .. gettext("Cancel") .. "]" ..
...
function tabbuilder.dialog_delete_world()
	return	"label[2,2;Delete World \"" .. filterlist.get_raw_list(worldlist)[menu.world_to_del].name .. "\"?]"..
			"button[3.5,4.2;2.6,0.5;world_delete_confirm;" .. gettext("Yes").. "]" ..
			"button[6,4.2;2.8,0.5;world_delete_cancel;" .. gettext("No") .. "]"
end
	...
	if fields["world_create_confirm"] or
		...
		if gameindex > 0 and
			...
			if not filterlist.uid_exists_raw(worldlist,worldname) then
				...
				message = gettext("A world named \"") .. worldname .. 
			...
			gamedata.errormessage = 
				gettext("No worldname given or no game selected")
		end
	end
...
function tabbuilder.tab_multiplayer()
	...
	local retval =
		"vertlabel[0,-0.25;".. gettext("CLIENT") .. "]" ..
		"label[1,-0.25;".. gettext("Favorites:") .. "]"..
		"label[1,4.25;".. gettext("Address/Port") .. "]"..
		"label[9,2.75;".. gettext("Name/Password") .. "]" ..
		"field[1.25,5.25;5.5,0.5;te_address;;" ..engine.setting_get("address") .."]" ..
		"field[6.75,5.25;2.25,0.5;te_port;;" ..engine.setting_get("port") .."]" ..
		"checkbox[1,3.6;cb_public_serverlist;".. gettext("Public Serverlist") .. ";" ..
		dump(engine.setting_getbool("public_serverlist")) .. "]"
		
	if not engine.setting_getbool("public_serverlist") then
		retval = retval .. 
		"button[6.45,3.95;2.25,0.5;btn_delete_favorite;".. gettext("Delete") .. "]"
	end
	
	retval = retval ..
		"button[9,4.95;2.5,0.5;btn_mp_connect;".. gettext("Connect") .. "]" ..
...
function tabbuilder.tab_server()
	...
	local retval = 
		"button[4,4.15;2.6,0.5;world_delete;".. gettext("Delete") .. "]" ..
		"button[6.5,4.15;2.8,0.5;world_create;".. gettext("New") .. "]" ..
		"button[9.2,4.15;2.55,0.5;world_configure;".. gettext("Configure") .. "]" ..
		"button[8.5,4.9;3.25,0.5;start_server;".. gettext("Start Game") .. "]" ..
		"label[4,-0.25;".. gettext("Select World:") .. "]"..
		"vertlabel[0,-0.25;".. gettext("START SERVER") .. "]" ..
		"checkbox[0.5,0.25;cb_creative_mode;".. gettext("Creative Mode") .. ";" ..
		dump(engine.setting_getbool("creative_mode")) .. "]"..
		"checkbox[0.5,0.7;cb_enable_damage;".. gettext("Enable Damage") .. ";" ..
		dump(engine.setting_getbool("enable_damage")) .. "]"..
		"checkbox[0.5,1.15;cb_server_announce;".. gettext("Public") .. ";" ..
		dump(engine.setting_getbool("server_announce")) .. "]"..
		"field[0.8,3.2;3,0.5;te_playername;".. gettext("Name") .. ";" ..
		engine.setting_get("name") .. "]" ..
		"pwdfield[0.8,4.2;3,0.5;te_passwd;".. gettext("Password") .. "]" ..
		"field[0.8,5.2;3,0.5;te_serverport;".. gettext("Server Port") .. ";30000]" ..
...
function tabbuilder.tab_settings()
	return	"vertlabel[0,0;SETTINGS]" ..
			...
			"checkbox[1,1.75;cb_3d_clouds;".. gettext("3D Clouds") .. ";"
					.. dump(engine.setting_getbool("enable_3d_clouds")) .. "]"..
			"checkbox[1,2.25;cb_opaque_water;".. gettext("Opaque Water") .. ";"
					.. dump(engine.setting_getbool("opaque_water")) .. "]"..
			"checkbox[1,2.75;old_style_modselection;".. gettext("Old style modmgr") .. ";" 
					.. dump(engine.setting_getbool("old_style_mod_selection"))		.. "]"..
			
			"checkbox[4,0.75;cb_mipmapping;".. gettext("Mip-Mapping") .. ";"
					.. dump(engine.setting_getbool("mip_map")) .. "]"..
			"checkbox[4,1.25;cb_anisotrophic;".. gettext("Anisotropic Filtering") .. ";"
					.. dump(engine.setting_getbool("anisotropic_filter")) .. "]"..
			"checkbox[4,1.75;cb_bilinear;".. gettext("Bi-Linear Filtering") .. ";"
					.. dump(engine.setting_getbool("bilinear_filter")) .. "]"..
			"checkbox[4,2.25;cb_trilinear;".. gettext("Tri-Linear Filtering") .. ";"
					.. dump(engine.setting_getbool("trilinear_filter")) .. "]"..
			
			"checkbox[7.5,0.75;cb_shaders;".. gettext("Shaders") .. ";"
					.. dump(engine.setting_getbool("enable_shaders")) .. "]"..
			"checkbox[7.5,1.25;cb_pre_ivis;".. gettext("Preload item visuals") .. ";"
					.. dump(engine.setting_getbool("preload_item_visuals"))	.. "]"..
			"checkbox[7.5,1.75;cb_particles;".. gettext("Enable Particles") .. ";"
					.. dump(engine.setting_getbool("enable_particles"))	.. "]"..
			"checkbox[7.5,2.25;cb_finite_liquid;".. gettext("Finite Liquid") .. ";"
					.. dump(engine.setting_getbool("liquid_finite")) .. "]"..
			
			"button[1,4.25;2.25,0.5;btn_change_keys;".. gettext("Change keys") .. "]"
end
...
function tabbuilder.tab_singleplayer()
	...
	return	"button[4,4.15;2.6,0.5;world_delete;".. gettext("Delete") .. "]" ..
			"button[6.5,4.15;2.8,0.5;world_create;".. gettext("New") .. "]" ..
			"button[9.2,4.15;2.55,0.5;world_configure;".. gettext("Configure") .. "]" ..
			"button[8.5,4.95;3.25,0.5;play;".. gettext("Play") .. "]" ..
			"label[4,-0.25;".. gettext("Select World:") .. "]"..
			"vertlabel[0,-0.25;".. gettext("SINGLE PLAYER") .. "]" ..
			"checkbox[0.5,0.25;cb_creative_mode;".. gettext("Creative Mode") .. ";" ..
			dump(engine.setting_getbool("creative_mode")) .. "]"..
			"checkbox[0.5,0.7;cb_enable_damage;".. gettext("Enable Damage") .. ";" ..
			dump(engine.setting_getbool("enable_damage")) .. "]"..
...
function tabbuilder.tab_texture_packs()
	local retval = "label[4,-0.25;".. gettext("Select texture pack:") .. "]"..
			"vertlabel[0,-0.25;".. gettext("TEXTURE PACKS") .. "]" ..
	...
	return	retval ..
			...
			"textarea[1.0,3.25;3.7,1.5;;"..(infotext or "")..";]"
end
...
function tabbuilder.tab_credits()
	return	"vertlabel[0,-0.5;CREDITS]" ..
			...
			"#FFFF00" .. gettext("Core Developers") .."," ..
			...
			"kahrl <kahrl@gmx.net>,"..
			","..
			"#FFFF00" .. gettext("Active Contributors") .. "," ..
			"sapier,"..
			...
			"," ..
			"#FFFF00" .. gettext("Previous Contributors") .. "," ..
