...
local function get_formspec(self)
	...
	if not self.hidden and (self.parent == nil or not self.parent.hidden) then
		...
		formspec = formspec ..
				self.tablist[self.last_tab_index].get_formspec(self,
...
local function handle_buttons(self, fields)
	...
	if self.tablist[self.last_tab_index].button_handler ~= nil then
		return
		self.tablist[self.last_tab_index].button_handler(self,
			fields,
			self.tablist[self.last_tab_index].name,
			self.tablist[self.last_tab_index].tabdata)
	end
...
local function handle_events(self, event)
	...
	if self.tablist[self.last_tab_index].evt_handler ~= nil then
		return
		self.tablist[self.last_tab_index].evt_handler(self,
			event,
			self.tablist[self.last_tab_index].name,
			self.tablist[self.last_tab_index].tabdata)
	end
...
local tabview_metatable = {
	...
	set_autosave_tab          = function(self, value) self.autosave_tab = value end,
