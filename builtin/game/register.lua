...
function core.register_item(name, itemdef)
	...
	if itemdef.type == "node" then
		...
		if not itemdef.tiles and itemdef.tile_images then
			core.log("deprecated", core.get_last_run_mod()..
				", tile_images is deprecated.")
	...
	if itemdef.can_place then
		core.override_on_place(name, function(itemstack, placer, pt)
			...
			return itemdef.can_place(vector.new(pos), node, itemstack, placer, pt)
		end)
	end
...
function core.register_alias(name, convert_to)
	...
	if core.registered_items[name] then
		core.log("warning", "Not registering alias for mod "..
			core.get_last_run_mod()..
			", item with same name is already defined: "..
			name .. " -> " .. convert_to)
	else
