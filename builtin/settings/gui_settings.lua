...
settings.all_pages = {}
settings.page_by_id = {}
settings.filtered_pages = settings.all_pages
settings.filtered_pages_by_id = settings.page_by_id

local component_funcs =  dofile(core.get_builtin_path() .. DIR_DELIM ..
		"settings" .. DIR_DELIM .. "components.lua")

local shadows_component =  dofile(core.get_builtin_path() .. DIR_DELIM ..
...
local function get_setting_info(name)
	for _, entry in ipairs(settings.full_settings) do
...
function settings.add_page(page)
	...
	assert(not settings.page_by_id[page.id], "Page " .. page.id .. " already registered")

	settings.all_pages[#settings.all_pages + 1] = page
	settings.page_by_id[page.id] = page
	return page
end
...
function settings.load(read_all, parse_mods)
	...
	settings.full_settings = settingtypes.parse_config_file(read_all, parse_mods)
	...
	if settings.page_by_id.controls_keyboard_and_mouse then
		table.insert(settings.page_by_id.controls_keyboard_and_mouse.content, 1, change_keys)
	do
		local content = settings.page_by_id.graphics_and_audio_shaders.content
...
function settings.update_filtered_pages(query)
	settings.filtered_pages = {}
	settings.filtered_pages_by_id = {}
	...
	for _, page in ipairs(settings.all_pages) do
		...
		if settings.page_has_contents(page, content) then
			local new_page = table.copy(page)
			new_page.content = content

			settings.filtered_pages[#settings.filtered_pages + 1] = new_page
			settings.filtered_pages_by_id[new_page.id] = new_page
...
function settings.page_has_contents(page, actual_content)
	local is_advanced =
...
function settings.get_formspec()
	local page_id = settings.page_id or "accessibility"
	local page = settings.filtered_pages_by_id[page_id]
	...
	local fs = {
		...
		"field[0.25,0.25;", tostring(search_width), ",0.75;search_query;;",
			core.formspec_escape(settings.query or ""), "]",
	...
	for _, other_page in ipairs(settings.filtered_pages) do
	...
	if #settings.filtered_pages == 0 then
	...
	if y >= tabsize.height - 1.25 then
		...
		fs[#fs + 1] = ("scrollbar[%f,1.25;%f,%f;vertical;leftscroll;%f]"):format(
				left_pane_width + 0.25, scrollbar_w, tabsize.height - 1.5, settings.leftscroll or 0)
	end

	fs[#fs + 1] = "style_type[button;border=;bgcolor=]"

	if not settings.components then
		settings.components = page and build_page_components(page) or {}
	end
	...
	if y >= tabsize.height then
		...
		fs[#fs + 1] = ("scrollbar[%f,0;%f,%f;vertical;rightscroll;%f]"):format(
				tabsize.width - scrollbar_w, scrollbar_w, tabsize.height, settings.rightscroll or 0)
	end
...
function create_settings_dlg()
	settings.load()
	local dlg = dialog_create("dlg_settings", settings.get_formspec, settings.buttonhandler, eventhandler)

	dlg.data.page_id = settings.update_filtered_pages("")
	
	return dlg
end

function settings.show_client_formspec(name, data)
	minetest.show_formspec(name, settings.get_formspec(data))
end
