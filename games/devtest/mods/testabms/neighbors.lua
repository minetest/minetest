...
minetest.register_node("testabms:missing_neighbor", {
	...
	on_construct = function (pos)
			local meta = minetest.get_meta(pos)
			meta:set_string("infotext", "Waiting for ABM testabms:missing_neighbor (normal drawtype testnode sensitive)")
		end,
})
...
minetest.register_abm({
		...
		action = function (pos)
			minetest.swap_node(pos, {name="testabms:after_abm"})
			local meta = minetest.get_meta(pos)
			meta:set_string("infotext", "ABM testabsm:missing_neighbor changed this node.")
		end
	})
...
minetest.register_node("testabms:required_missing_neighbor", {
	...
	on_construct = function (pos)
			local meta = minetest.get_meta(pos)
			meta:set_string("infotext", "Waiting for ABM testabms:required_missing_neighbor (wint normal drawtype testnode and no glasslike drawtype testnode sensitive)")
		end,
})
...
minetest.register_abm({
		...
		action = function (pos)
			minetest.swap_node(pos, {name="testabms:after_abm"})
			local meta = minetest.get_meta(pos)
			meta:set_string("infotext", "ABM testabsm:required_missing_neighbor changed this node.")
		end
	})
