...
* spec = SimpleSoundSpec (see lua-api.md)
* looped = bool
...
`core.get_games() -> table` (possible in async calls)
* return a tale of all games in upper format

`core.get_mapgen_names([include_hidden=false]) -> table` (possible in async calls)
* returns a table of map generator algorithms registered in the core
