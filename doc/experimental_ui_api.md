...
implemented in the current version of the UI API, such as scrollbars, edit
fields, or flex sizers. These are included in the documentation since they are
particularly useful for explaining some of the features of the UI API. The
documentation always makes a note of when it refers to unsupported features.
...
ui.Elem {
    children = {
        ui.Label "child_1" {label="First"},
        ui.Label "child_2" {label="Second"},
    },
}
...
elements in the element tree descend. Since the root element has no parent, it
is positioned relative to the entire screen. The root element must be of type
`ui.Root`.
...
* `bg`: Used for things that need to be drawn before everything else, such as
  vignettes or crosshairs.
...
* `chat`: Used to display GUI-like popup or chat messages, but that can't be
  interacted with. Hidden when the chat is hidden.
* `gui`: Used for GUIs that the user can interact with.
* `fg`: Used for things that need to be drawn after everything else, in case
  anything needs to cover formspecs or GUI windows.
...
The `gui` and `chat` window types use `gui_scaling` for the base pixel size,
...
also have a list of group IDs that allow selectors to style multiple elements
...
local style = ui.Style "my_mod:toggle" {
    -- Properties for styling `MyToggle`.
}

local toggle = MyToggle "toggle_id" {
...
closed if it's not uncloseable, which will naturally cause the `on_close` event
to be fired. The enter key will cause the `on_submit` event to be sent to the
server, which the server may use for whatever purpose it desires, such as by
closing the window and using the form's data somewhere.
...
details on selectors and the list of supported style properties. Useful style
properties relevant to layout and display are also discussed in the [Layout and
visuals] section.
...
Future versions will also support automatic positioning via sizers. Moreover,
every box supports a universal set of style properties for visuals, rather than
the limited and element-dependent styling of formspecs.

See `StyleSpec` for a full list of supported style properties.

### Scaled pixels

_Pixels_ in the UI API refer to DPI and user scaled pixels as defined by
`core.get_player_window_information()`, i.e. each pixel is either
...
padding, margins, and nine-slice image borders. This allows players to specify
UI scaling that is adjusted to their screen and/or accessibility needs, but
without sacrificing the flexibility afforded by pixel dimensioning for modders.
...
paddings and margins can be applied to add space inside boxes and around their
parents and children. This leads to a number of conceptual rectangular areas
...
bounding rectangles, such sizers that automatically position their children or
a moving bounding rectangle like a scrollbar thumb.

After the box is positioned with normalized coordinates within its bounding
rectangle (as described above), the resulting positioned rectangle is called
the _layout rectangle_. The contents of the layout rectangle are inset by the
`margin` property, making it possible to add space between sibling boxes.
...
display rectangle is where mouse and touch input is registered.
...
the icon rectangle, which will cause the content to be displayed on top. If
they are not set to overlap, the `icon_gutter` property can be used to control
how much space will be placed between them.
...
too primitive to position elements in complex UIs.
...
described above. However, there are other properties that bear mentioning.

By default, if a box extends outside of its bounding rectangle, then that part
of the box and its content is not drawn. Moreover, mouse and touch input will
not register on the clipped part of the box. However, if the `noclip` property
is set, then the box will never be clipped.

To turn off drawing for a box, the `visible` property can be set to false,
which will cause the box itself to not be drawn. However, the box's contents
will still be drawn and the box will still respond to touch and mouse input.
...
* `ui.get_open_contexts()`: Returns a table containing the contexts for all
    ...
    * `builder` (function): The builder function for the context. This function
      takes four parameters, `function(context, player, state, param)`:
        ...
        * `param` (table): The parameter table for this call to the builder
          function.
      The builder function should return a freshly created `ui.Window` object.
...
* `ui.Image`
    ...
    * A static element that only meant for displaying an image, either static
      or animated. Generally, the `icon_image` style property should be used to
      display the image, but if the image needs to be stretched rather than
      have its aspect ratio preserved, the `box_image` property may be used.

### Theming

For `ui.Image`, the prelude sets `fg_image` to zero to make the image fill as
...
* ID required: No
* Boxes:
    * `backdrop` (static): The backdrop of the root element, which is
      positioned relative to the bounding rectangle of the screen. It is the
      parent of the `main` box.
...
users must explicitly choose a size with `size` and/or `rel_size`.
...
* `rel_pos` (2D vector): The position of the box in normalized coordinates
  relative to the bounding rectangle. Default `{0, 0}`.
...
* `visible` (boolean): Determines if the box should be drawn. This does not
  affect the visibility of any of the box's children. Default true.
* `noclip` (boolean): Determines whether the box should be clipped to the
