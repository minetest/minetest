...
Improved performance in SQLite3 for existing worlds can be obtained by
recreating the table with a `pos INTEGER PRIMARY KEY` instead.
An existing database can (while Luanti is *not* running) be optimized using
```sql
CREATE TABLE `blocks_tmp` (`pos` INTEGER PRIMARY KEY, `data` BLOB);
INSERT INTO  `blocks_tmp` SELECT * from `blocks`;
DROP   TABLE `blocks`;
ALTER  TABLE `blocks_tmp` rename to `blocks`;
VACUUM;
```
You need enough free diskspace for a full copy of the database during conversion.
...
A position encoding can be converted back using:
