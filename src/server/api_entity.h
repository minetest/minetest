...
public:
   ...
   virtual void luaentity_Activate (u16 id, const std::string &staticdata, u32 dtime_s) {}
   virtual void luaentity_Deactivate(u16 id) {}
   ...
   virtual void luaentity_Step(
			u16 id, float dtime, const collisionMoveResult *moveresult)
	{
	}
   virtual bool luaentity_Punch(u16 id, ServerActiveObject *puncher,
			float time_from_last_punch, const ToolCapabilities *toolcap,
			v3f dir, s16 damage)
	{
		return false;
	}
   virtual bool luaentity_on_death(u16 id, ServerActiveObject *killer) { return false; }
   virtual void luaentity_Rightclick(u16 id, ServerActiveObject *clicker) {}
