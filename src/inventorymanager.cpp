		...
		InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
	...
	*/
	bool ignore_rollback =
			(from_inv.type == InventoryLocation::PLAYER && from_inv == to_inv);
	...
	*/
	if (!ignore_rollback && gamedef->rollback()) {
		...
		if (dst_can_put_count != -1) {
			...
			action.setModifyInventoryStack(loc, to_list, to_i, true, src_item);
			rollback->reportAction(action);
		}
	}
	...
	*/
	...
	if (from_inv.type == InventoryLocation::DETACHED && from_inv == to_inv) {
		...
		// Destination is detached
		if (to_inv.type == InventoryLocation::DETACHED) {
			PLAYER_TO_SA(player)->detached_inventory_OnPut(*this, src_item, player);
		}
		// Source is detached
		if (from_inv.type == InventoryLocation::DETACHED) {
	...
	if (from_inv.type == InventoryLocation::NODEMETA && from_inv == to_inv) {
		...
		// Destination is nodemeta
		if (to_inv.type == InventoryLocation::NODEMETA) {
			PLAYER_TO_SA(player)->nodemeta_inventory_OnPut(*this, src_item, player);
		}
		// Source is nodemeta
		if (from_inv.type == InventoryLocation::NODEMETA) {
	...
	if (from_inv.type == InventoryLocation::PLAYER && from_inv == to_inv) {
		...
		if (to_inv.type == InventoryLocation::PLAYER) {
			PLAYER_TO_SA(player)->player_inventory_OnPut(*this, src_item, player);
		}
		...
		InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
	...
	*/
	...
	case InventoryLocation::PLAYER:
		src_can_take_count =
				PLAYER_TO_SA(player)->player_inventory_AllowTake(*this, src_item, player);
		break;
	default:
		break;
	}
		...
		InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
	...
	*/
	...
	while (found && list_craftresult->itemFits(0, crafted)) {
		...
		actionstream << player->getDescription() << " crafts " << crafted.getItemString()
					 << std::endl;
