...
void MinimapUpdateThread::doUpdate()
{
	...
	while (popBlockUpdate(&update)) {
		if (update.data) {
			...
			std::pair<std::map<v3s16, MinimapMapblock *>::iterator, bool> result =
					m_blocks_cache.insert(std::make_pair(update.pos, update.data));
			if (!result.second) {
...
Minimap::Minimap(Client *client)
{
	...
	data->minimap_mask_round = driver->createImage(
			m_tsrc->getTexture("minimap_mask_round.png"), core::position2d<s32>(0, 0),
	...
	data->minimap_mask_square = driver->createImage(
			m_tsrc->getTexture("minimap_mask_square.png"), core::position2d<s32>(0, 0),
			core::dimension2d<u32>(MINIMAP_MAX_SX, MINIMAP_MAX_SY));
...
void Minimap::setMinimapMode(MinimapMode mode)
{
	static const MinimapModeDef modedefs[MINIMAP_MODE_COUNT] = { { false, 0, 0 },
		...
		{ false, m_surface_mode_scan_height, 64 }, { true, 32, 128 }, { true, 32, 64 },
		{ true, 32, 32 } };
	...
	for (s16 x = 0; x < data->map_size; x++)
		for (s16 z = 0; z < data->map_size; z++) {
			...
			if (mmpixel->air_count > 0)
				c.setGreen(
						core::clamp(core::round32(32 + mmpixel->air_count * 8), 0, 255));
			else
				c.setGreen(0);
		...
		video::IImage *map_image, video::IImage *heightmap_image)
{
	...
	for (s16 x = 0; x < data->map_size; x++)
		for (s16 z = 0; z < data->map_size; z++) {
			...
			heightmap_image->setPixel(
					x, data->map_size - z - 1, video::SColor(255, h, h, h));
		}
}
...
video::ITexture *Minimap::getMinimapTexture()
{
	...
	video::IImage *minimap_mask = data->minimap_shape_round ? data->minimap_mask_round :
...
{
	if (data->minimap_shape_round) {
		return v3f(std::cos(m_angle * core::DEGTORAD), std::sin(m_angle * core::DEGTORAD),
				1.0);
	}
...
void Minimap::drawMinimap()
{
	...
	for (std::list<v2f>::const_iterator i = m_active_markers.begin();
		...
		driver->draw2DImage(
				data->object_marker_red, dest_rect, img_rect, &dest_rect, &c[0], true);
	}
}
...
void Minimap::updateActiveMarkers()
{
	...
	for (Nametag *nametag : nametags) {
		v3s16 pos = floatToInt(nametag->parent_node->getAbsolutePosition() +
						intToFloat(client->getCamera()->getOffset(), BS),
				BS);
		pos -= data->pos -
				v3s16(data->map_size / 2, data->scan_height / 2, data->map_size / 2);
