...
Particle::Particle(IGameDef *gamedef, LocalPlayer *player, ClientEnvironment *env,
		v3f pos, v3f velocity, v3f acceleration, float expirationtime, float size,
		bool collisiondetection, bool collision_removal, bool object_collision,
		bool vertical, video::ITexture *texture, v2f texpos, v2f texsize,
		const struct TileAnimationParams &anim, u8 glow, video::SColor color) :
		scene::ISceneNode(
...
ParticleSpawner::ParticleSpawner(IGameDef *gamedef, LocalPlayer *player, u16 amount,
		float time, v3f minpos, v3f maxpos, v3f minvel, v3f maxvel, v3f minacc,
		v3f maxacc, float minexptime, float maxexptime, float minsize,
		float maxsize, bool collisiondetection, bool collision_removal,
		bool object_collision, u16 attached_id, bool vertical,
		video::ITexture *texture, const struct TileAnimationParams &anim, u8 glow,
		...
		bool is_attached, const v3f &attached_pos, float attached_yaw)
{
	...
	float exptime = rand() / (float)RAND_MAX * (m_maxexptime - m_minexptime) +
			m_minexptime;
	float size = rand() / (float)RAND_MAX * (m_maxsize - m_minsize) + m_minsize;

	m_particlemanager->addParticle(new Particle(m_gamedef, m_player, env, pos, vel,
			acc, exptime, size, m_collisiondetection, m_collision_removal,
			m_object_collision, m_vertical, m_texture, v2f(0.0, 0.0),
			v2f(1.0, 1.0), m_animation, m_glow));
}
	...
	if (m_spawntime != 0) {
		...
		for (std::vector<float>::iterator i = m_spawntimes.begin();
			...
			if ((*i) <= m_time && m_amount > 0) {
				...
				// player.
				if (!unloaded)
					spawnParticle(env, radius, is_attached,
							attached_pos, attached_yaw);
		...
		for (int i = 0; i <= m_amount; i++) {
			if (rand() / (float)RAND_MAX < dtime)
				spawnParticle(env, radius, is_attached, attached_pos,
						attached_yaw);
		}
	}
}
		...
		ClientEvent *event, Client *client, LocalPlayer *player)
{
	...
	case CE_DELETE_PARTICLESPAWNER: {
		...
		if (m_particle_spawners.find(event->delete_particlespawner.id) !=
			...
			delete m_particle_spawners.find(event->delete_particlespawner.id)
					->second;
	...
	case CE_ADD_PARTICLESPAWNER: {
		{
			...
			if (m_particle_spawners.find(event->add_particlespawner.id) !=
					m_particle_spawners.end()) {
				delete m_particle_spawners
						.find(event->add_particlespawner.id)
						->second;
		...
		auto toadd = new ParticleSpawner(client, player,
				...
				*event->add_particlespawner.maxacc,
				...
				event->add_particlespawner.vertical, texture,
				event->add_particlespawner.animation,
				event->add_particlespawner.glow, this);
	...
	case CE_SPAWN_PARTICLE: {
		...
		Particle *toadd = new Particle(client, player, m_env,
				*event->spawn_particle.pos, *event->spawn_particle.vel,
				*event->spawn_particle.acc,
				...
				event->spawn_particle.vertical, texture, v2f(0.0, 0.0),
				v2f(1.0, 1.0), event->spawn_particle.animation,
		...
		const MapNode &n, const ContentFeatures &f)
{
	...
	// Physics
	v3f velocity((rand() % 150) / 50.0f - 1.5f, (rand() % 150) / 50.0f,
			(rand() % 150) / 50.0f - 1.5f);
	v3f acceleration(0.0f,
			-player->movement_gravity * player->physics_override_gravity / BS,
			0.0f);
	v3f particlepos = v3f((f32)pos.X + (rand() % 100) / 200.0f - 0.25f,
	...
	Particle *toadd = new Particle(gamedef, player, m_env, particlepos, velocity,
			acceleration,
			(rand() % 100) / 100.0f, // expiration time
			visual_size, true, false, false, false, texture, texpos, texsize,
			anim, 0, color);
