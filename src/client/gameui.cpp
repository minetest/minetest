...
void GameUI::setChatText(const EnrichedString &chat_text, u32 recent_chat_count)
{
	...
	s32 width = std::min(g_fontengine->getTextWidth(chat_text.c_str()) + 10,
			window_size.X - 20);
	m_guitext_chat->setRelativePosition(
			core::rect<s32>(10, chat_y, width, chat_y + window_size.Y));

	// now use real height of text and adjust rect according to this size
	m_guitext_chat->setRelativePosition(core::rect<s32>(
			10, chat_y, width, chat_y + m_guitext_chat->getTextHeight()));

	// Don't show chat if disabled or empty or profiler is enabled
	m_guitext_chat->setVisible(m_flags.show_chat && recent_chat_count != 0 &&
...
void GameUI::updateProfiler()
{
	if (m_profiler_current_page != 0) {
		...
		s32 screen_height = RenderingEngine::get_video_driver()
						    ->getScreenSize()
						    .Height;
