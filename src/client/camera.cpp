...
void Camera::addArmInertia(f32 player_yaw)
{
	...
	if (m_cam_vel.X > 1.0f || m_cam_vel.Y > 1.0f) {
		...
		*/
		...
		if (m_cam_vel.Y > 1.0f) {
			...
			m_wieldmesh_offset.Y = rangelim(m_wieldmesh_offset.Y,
					WIELDMESH_OFFSET_Y - 10.0f,
					WIELDMESH_OFFSET_Y + 5.0f);
		}
		...
		*/
		...
		f32 dec_X = 0.12f * (m_cam_vel_old.X * (1.0f + (1.0f - m_arm_dir.X))) *
			    (gap_X / 20.0f);

		f32 dec_Y = 0.06f * (m_cam_vel_old.Y * (1.0f + (1.0f - m_arm_dir.Y))) *
			    (gap_Y / 15.0f);
		...
		LocalPlayer *player, f32 frametime, f32 busytime, f32 tool_reload_ratio)
{
	...
	m_headnode->setPosition(
			PlayerEyeOffset +
			v3f(0, cameratilt * -player->hurt_tilt_strength + fall_bobbing,
					0));
	m_headnode->setRotation(v3f(
			player->getPitch(), 0, cameratilt * player->hurt_tilt_strength));
	...
	if (m_camera_mode > CAMERA_MODE_FIRST) {
		...
		for (int i = BS; i <= BS * 2.75; i++) {
			...
			MapNode n = m_client->getEnv().getClientMap().getNodeNoEx(
					floatToInt(my_cp, BS));
