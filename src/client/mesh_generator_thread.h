...
public:
	...
	void addBlock(Map *map, v3POS p, bool ack_block_to_server, bool urgent);
...
private:
	...
	std::map<v3POS, CachedMapBlockData *> m_cache;
	std::mutex m_mutex;
	...
	CachedMapBlockData *cacheBlock(Map *map, v3POS p, UpdateMode mode,
			size_t *cache_hit_counter = NULL);
	CachedMapBlockData *getCachedBlock(const v3POS &p);
...
public:
	...
	// Caches the block at p and its neighbors (if needed) and queues a mesh
	// update for the block at p
	void updateBlock(Map *map, v3POS p, bool ack_block_to_server, bool urgent);
