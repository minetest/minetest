...
void ClientMediaDownloader::addFile(const std::string &name, const std::string &sha1)
{
	...
	if (sha1.size() != 20) {
		errorstream
				<< "Client: ignoring illegal SHA1 sent by server: " << hex_encode(sha1)
				<< " \"" << name << "\"" << std::endl;
		return;
	}
...
void ClientMediaDownloader::initialStep(Client *client)
{
	...
	for (auto &file_it : m_files) {
		...
		if (found_in_cache) {
			bool success = checkAndLoad(name, sha1, tmp_os.str(), true, client);
			if (success) {
	...
	if (m_uncached_count != 0) {
		...
		if (!did) {
			errorstream
					<< "Client: "
					<< "Could not create media cache directory: " << getMediaCacheDir()
					<< std::endl;
		}
	}
	...
	if (!USE_CURL || m_uncached_count == 0 || m_remotes.empty()) {
		...
		for (u32 i = 0; i < m_remotes.size(); ++i) {
			...
			actionstream << "Client: Contacting remote server \"" << remote->baseurl
						 << "\"" << std::endl;
			...
			if (addr.find(':', 0) != std::string::npos)
				addr = '[' + addr + ']';
			fetch_request.extra_headers.emplace_back(std::string("Referer: minetest://") +
					addr + ":" + std::to_string(client->getServerAddress().getPort()));
	...
	if (fetch_result.succeeded) {
		try {
			...
			infostream << "Client: Remote server \"" << remote->baseurl
					   << "\" sent invalid hash set: " << e.what() << std::endl;
		}
	}
}
		...
		const HTTPFetchResult &fetch_result, Client *client)
{
	...
	if (fetch_result.succeeded) {
		bool success =
				checkAndLoad(name, filestatus->sha1, fetch_result.data, false, client);
		if (success) {
			filestatus->received = true;
	...
	for (std::map<std::string, FileStatus *>::iterator files_iter =
		...
		if (!filestatus->received && filestatus->current_remote < 0) {
			...
			if (remote_id >= 0) {
				...
				std::string url = remote->baseurl + hex_encode(filestatus->sha1);
				...
				m_remote_file_transfers.insert(std::make_pair(m_httpfetch_next_id, name));
		...
		if (filestatus->received ||
				(filestatus->current_remote < 0 && !m_outstanding_hash_sets)) {
		...
		const std::string &name, const std::string &data, Client *client)
{
	...
	if (file_iter == m_files.end()) {
		errorstream << "Client: server sent media file that was"
					<< "not announced, ignoring it: \"" << name << "\"" << std::endl;
		return;
	}
		...
		const std::string &data, bool is_from_cache, Client *client)
{
	...
	if (data_sha1 != sha1) {
		...
		infostream << "Client: " << cached_or_received_uc << " media file " << sha1_hex
				   << " \"" << name << "\" "
	...
	if (!success) {
		infostream << "Client: "
				   << "Failed to load " << cached_or_received << " media: " << sha1_hex
				   << " \"" << name << "\"" << std::endl;
		return false;
	}
