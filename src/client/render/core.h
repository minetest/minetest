...
/*
	...
	So we have to check the runtime environment
	and choose the proper symbol.
*/
...
public:
	...
	COpenGLRenderSurface(const core::dimension2d<u32> &size,
			     const io::path &name,
			     const bool isDepth = false,
			     irr::video::ECOLOR_FORMAT format = irr::video::ECF_UNKNOWN);

	virtual ~COpenGLRenderSurface();

	virtual void *lock(irr::video::E_TEXTURE_LOCK_MODE mode = irr::video::ETLM_READ_WRITE,
			   u32 mipmapLevel = 0) { return NULL; }
...
{
public:
	COpenGLRenderTarget(const core::dimension2d<u32> &size,
			    const io::path &name,
