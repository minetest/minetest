		...
		bool _draw_wield_tool, bool _draw_crosshair, Sky &sky, Clouds &clouds)
{
...

	{
		...
		if (lightScene == NULL) {
			...
			lightScene = new COpenGLRenderTarget(v2u32(SHADOWMAP_RES, SHADOWMAP_RES), "lightscene", true, driver);
		}
...
COpenGLRenderSurface::COpenGLRenderSurface(const core::dimension2d<u32> &size,
					   const io::path &name,
					   const bool isDepth,
					   irr::video::ECOLOR_FORMAT format) : ITexture(name)
{
	ImageSize = size;
	TextureSize = size;
	...
	if (!isDepth) {
		...
		glTexImage2D(GL_TEXTURE_2D,
			     0,
			     GL_DEPTH24_STENCIL8_EXT /* GL_DEPTH_COMPONENT24 */,
			     ImageSize.Width,
			     ImageSize.Height,
			     0,
			     GL_DEPTH_COMPONENT,
...
COpenGLRenderTarget::COpenGLRenderTarget(const core::dimension2d<u32> &size,
					 const io::path &name,
