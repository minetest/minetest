		...
		v3s16 cam_pos_nodes, v3s16 *p_blocks_min, v3s16 *p_blocks_max)
{
	...
	// world edges.
	v3s32 p_nodes_min(cam_pos_nodes.X - box_nodes_d.X, cam_pos_nodes.Y - box_nodes_d.Y,
			cam_pos_nodes.Z - box_nodes_d.Z);
	v3s32 p_nodes_max(cam_pos_nodes.X + box_nodes_d.X, cam_pos_nodes.Y + box_nodes_d.Y,
			cam_pos_nodes.Z + box_nodes_d.Z);
	// Take a fair amount as we will be dropping more out later
	// Umm... these additions are a bit strange but they are needed.
	*p_blocks_min = v3s16(p_nodes_min.X / MAP_BLOCKSIZE - 3,
			p_nodes_min.Y / MAP_BLOCKSIZE - 3, p_nodes_min.Z / MAP_BLOCKSIZE - 3);
	*p_blocks_max = v3s16(p_nodes_max.X / MAP_BLOCKSIZE + 1,
			p_nodes_max.Y / MAP_BLOCKSIZE + 1, p_nodes_max.Z / MAP_BLOCKSIZE + 1);
}
		...
		if (pass == scene::ESNRP_SOLID) {
			...
			if (mapBlockMesh->isAnimationForced() || !faraway ||
				...
				bool animated = mapBlockMesh->animate(
						faraway, animation_time, crack, daynight_ratio);
				if (animated)
					mesh_animate_count++;
			} else {
			...
			for (int layer = 0; layer < MAX_TILE_LAYERS; layer++) {
				...
				for (u32 i = 0; i < c; i++) {
					...
					if (transparent == is_transparent_pass) {
						...
						material.setFlag(
								video::EMF_TRILINEAR_FILTER, m_cache_trilinear_filter);
						material.setFlag(
								video::EMF_BILINEAR_FILTER, m_cache_bilinear_filter);
						material.setFlag(
								video::EMF_ANISOTROPIC_FILTER, m_cache_anistropic_filter);
		...
		int *result, bool *sunlight_seen)
{
	...
	for (int i = 0; distance < end_distance; i++) {
		...
		if (ndef->get(n).param_type != CPT_LIGHT ||
				(!ndef->get(n).sunlight_propagates && !allow_non_sunlight_propagates)) {
			nonlight_seen = true;
			noncount++;
			if (noncount >= 4)
				break;
...
int ClientMap::getBackgroundBrightness(
		float max_d, u32 daylight_factor, int oldvalue, bool *sunlight_seen_result)
{
	...
	if (z_directions[0].X < -99) {
		for (u32 i = 0; i < sizeof(z_directions) / sizeof(*z_directions); i++) {
			...
			z_directions[i] = v3f(
					0.02 * myrand_range(-100, 100), 1.0, 0.01 * myrand_range(-100, 100))
									  .normalize();
...
void ClientMap::renderPostFx(CameraMode cam_mode)
{
	...
	if (features.solidness == 2 &&
			!(g_settings->getBool("noclip") && m_client->checkLocalPrivilege("noclip")) &&
