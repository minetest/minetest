...
public:
	...
	ExtrusionMeshCache()
	{
		for (int resolution = MIN_EXTRUSION_MESH_RESOLUTION;
				resolution <= MAX_EXTRUSION_MESH_RESOLUTION; resolution *= 2) {
			m_extrusion_meshes[resolution] = createExtrusionMesh(resolution, resolution);
		}
...
WieldMeshSceneNode::WieldMeshSceneNode(scene::ISceneManager *mgr, s32 id, bool lighting) :
	...
	m_material_type(video::EMT_TRANSPARENT_ALPHA_CHANNEL_REF), m_lighting(lighting)
{
		...
		Client *client, content_t id, std::vector<ItemPartColor> *colors)
{
	...
	for (auto &prebuffers : collector.prebuffers)
		for (PreMeshBuffer &p : prebuffers) {
			...
			mesh->addMeshBuffer(buf);
			buf->append(
					&p.vertices[0], p.vertices.size(), &p.indices[0], p.indices.size());
			buf->drop();
			colors->push_back(ItemPartColor(p.layer.has_color, p.layer.color));
		}
	return mesh;
}
		...
		const ItemStack &item, Client *client, bool check_wield_image)
{
	...
	if (m_enable_shaders) {
		u32 shader_id =
				shdrsrc->getShader("object_shader", TILE_MATERIAL_BASIC, NDT_NORMAL);
	...
	if (def.type == ITEM_NODE) {
		if (f.mesh_ptr[0]) {
			...
			postProcessNodeMesh(
					mesh, f, m_enable_shaders, true, &m_material_type, &m_colors);
			changeToMesh(mesh);
			mesh->drop();
		...
		setExtruded(def.inventory_image, def.inventory_overlay, def.wield_scale, tsrc, 1);
		m_colors.emplace_back();
		// overlay is white, if present
		m_colors.emplace_back(true, video::SColor(0xFFFFFFFF));
		return;
...
void getItemMesh(Client *client, const ItemStack &item, ItemMesh *result)
{
	...
	FATAL_ERROR_IF(
			!g_extrusion_mesh_cache, "Extrusion mesh cache is not yet initialized");
	...
	if (!def.inventory_image.empty()) {
		...
		if (f.mesh_ptr[0]) {
			...
			case NDT_FLOWINGLIQUID: {
				...
				// add overlays
				postProcessNodeMesh(
						mesh, f, false, false, nullptr, &result->buffer_colors);
				break;
			}
			default: {
	...
	for (u32 i = 0; i < mc; ++i) {
		...
		for (int layernum = 0; layernum < MAX_TILE_LAYERS; layernum++) {
			...
			if (layernum != 0) {
				...
				copy->drop();
				buf = copy;
				colors->push_back(ItemPartColor(layer->has_color, layer->color));
			} else {
