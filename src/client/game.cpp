...
bool Game::createSingleplayerServer(const std::string &map_dir,
		const SubgameSpec &gamespec, u16 &port)
{
	infostream << "Creating server on port " << port << std::endl;
	showOverlayMessage(N_("Creating server on..."), 0, 5);
	...
	// If the port was 0 the operating system selects a port, feed that information back to the
	// caller.
	infostream << "Listening on port " << port << std::endl;
	port = server->port();
