...
struct TextDestNodeMetadata : public TextDest
{
	...
	void gotText(const std::wstring &text)
	{
		...
		infostream << "Submitting 'text' field of node at (" << m_p.X << "," << m_p.Y
				   << "," << m_p.Z << "): " << ntext << std::endl;
		StringMap fields;
		fields["text"] = ntext;
...
struct LocalFormspecHandler : public TextDest
{
	...
	LocalFormspecHandler(const std::string &formname, Client *client) : m_client(client)
	{
		m_formname = formname;
	}
...
public:
	...
	SoundMaker(ISoundManager *sound, const NodeDefManager *ndef)
		: m_sound(sound), m_ndef(ndef), makes_footstep_sound(true),
		  m_player_step_timer(0.0f), m_player_jump_timer(0.0f)
	{
	}
...
public:
	...
	{
		reinterpret_cast<GameGlobalShaderConstantSetter *>(userdata)->onSettingsChange(
				name);
	}
	...
	GameGlobalShaderConstantSetter(
		...
		: m_sky(sky), m_force_fog_off(force_fog_off), m_fog_range(fog_range),
		  m_sky_bg_color("skyBgColor"), m_fog_distance("fogDistance"),
		  m_animation_timer_vertex("animationTimer"),
		  m_animation_timer_pixel("animationTimer"), m_day_light("dayLight"),
		  m_eye_position_pixel("eyePosition"), m_eye_position_vertex("eyePosition"),
		  m_minimap_yaw("yawVec"), m_camera_offset_pixel("cameraOffset"),
		  m_camera_offset_vertex("cameraOffset"), m_base_texture("baseTexture"),
		  m_normal_texture("normalTexture"), m_texture_flags("textureFlags"),
		  m_client(client)
	{
...
public:
	GameGlobalShaderConstantSetterFactory(
		...
		: m_sky(NULL), m_force_fog_off(force_fog_off), m_fog_range(fog_range),
		  m_client(client)
	{
	}
...
public:
	...
	bool startup(bool *kill, bool random_input, InputHandler *input,
			const std::string &map_dir, const std::string &playername,
			const std::string &password,
			// If address is "", local server is used and address is updated
			std::string *address, u16 port, std::string &error_message, bool *reconnect,
...
protected:
	...
	bool init(const std::string &map_dir, std::string *address, u16 port,
			const SubgameSpec &gamespec);
	bool initSound();
	bool createSingleplayerServer(const std::string &map_dir, const SubgameSpec &gamespec,
			u16 port, std::string *address);
	...
	bool connectToServer(const std::string &playername, const std::string &password,
			std::string *address, u16 port, bool *connect_ok, bool *aborted);
	...
	void handlePointingAtNode(const PointedThing &pointed, const ItemStack &selected_item,
			const ItemStack &hand_item, f32 dtime);
	...
	void showOverlayMessage(
			const char *msg, float dtime, int percent, bool draw_clouds = true);
...
bool Game::startup(bool *kill, bool random_input, InputHandler *input,
		const std::string &map_dir, const std::string &playername,
		const std::string &password,
		std::string *address, // can change if simple_singleplayer_mode
		u16 port, std::string &error_message, bool *reconnect, ChatBackend *chat_backend,
...
void Game::run()
{
...
#ifdef __ANDROID__
	m_cache_hold_aux1 =
			g_settings->getBool("fast_move") && client->checkPrivilege("fast");
#endif
	...
	while (RenderingEngine::run() &&
		...
		cam_view.camera_yaw += (cam_view_target.camera_yaw - cam_view.camera_yaw) *
				m_cache_cam_smoothing;
		cam_view.camera_pitch += (cam_view_target.camera_pitch - cam_view.camera_pitch) *
				m_cache_cam_smoothing;
		...
		processPlayerInteraction(
				dtime, m_game_ui->m_flags.show_hud, m_game_ui->m_flags.show_debug);
...
bool Game::init(const std::string &map_dir, std::string *address, u16 port,
		const SubgameSpec &gamespec)
{
		...
		const SubgameSpec &gamespec, u16 port, std::string *address)
{
	...
	try {
		...
		infostream << "Resolving bind address \"" << bind_str << "\" failed: " << e.what()
...
#endif
	if (!connectToServer(
				playername, password, address, port, &could_connect, &connect_aborted))
		return false;
...
bool Game::initGui()
{
	...
	gui_chat_console = new GUIChatConsole(
			guienv, guienv->getRootGUIElement(), -1, chat_backend, client, &g_menumgr);
	if (!gui_chat_console) {
...
bool Game::connectToServer(const std::string &playername, const std::string &password,
		std::string *address, u16 port, bool *connect_ok, bool *connection_aborted)
{
	...
	*connection_aborted = false;
	...
	if (connect_address.isIPv6() && !g_settings->getBool("enable_ipv6")) {
		*error_message = "Unable to connect to " + connect_address.serializeString() +
				" because IPv6 is disabled";
		errorstream << *error_message << std::endl;
		return false;
	}

	client = new Client(playername.c_str(), password, *address, *draw_control,
			texture_src, shader_src, itemdef_manager, nodedef_manager, sound, eventmgr,
			...
			if (client->m_is_registration_confirmation_state) {
				if (registration_confirmation_shown) {
					...
					(new GUIConfirmRegistration(guienv, guienv->getRootGUIElement(), -1,
							 &g_menumgr, client, playername, password, connection_aborted,
							 texture_src))
							->drop();
				}
			} else {
...
bool Game::getServerContent(bool *aborted)
{
	...
	while (RenderingEngine::run()) {
		...
		if (!client->itemdefReceived()) {
			...
			progress = 25;
			RenderingEngine::draw_load_screen(text, guienv, texture_src, dtime, progress);
			delete[] text;
			...
			RenderingEngine::draw_load_screen(
					utf8_to_wide(message.str()), guienv, texture_src, dtime, progress);
		}
	}
...
inline bool Game::handleCallbacks()
{
	...
	if (g_gamecallback->changepassword_requested) {
		(new GUIPasswordChange(guienv, guiroot, -1, &g_menumgr, client, texture_src))
				->drop();
		g_gamecallback->changepassword_requested = false;
	}

	if (g_gamecallback->changevolume_requested) {
		(new GUIVolumeChange(guienv, guiroot, -1, &g_menumgr, texture_src))->drop();
		g_gamecallback->changevolume_requested = false;
	}

	if (g_gamecallback->keyconfig_requested) {
		(new GUIKeyChangeMenu(guienv, guiroot, -1, &g_menumgr, texture_src))->drop();
...
void Game::processKeyInput()
{
	if (wasKeyDown(KeyType::DROP)) {
		...
		if (g_settings->getBool("enable_sound")) {
			float new_volume =
					rangelim(g_settings->getFloat("sound_volume") + 0.1f, 0.0f, 1.0f);
			wchar_t buf[100];
...
void Game::toggleNoClip()
{
	...
	if (noclip) {
		if (client->checkPrivilege("noclip")) {
			...
			m_game_ui->showTranslatedStatusText(
					"Noclip mode enabled (note: no 'noclip' privilege)");
		}
	} else {
...
void Game::toggleDebug()
{
	...
	if (!m_game_ui->m_flags.show_debug) {
		...
		if (client->checkPrivilege("debug")) {
			m_game_ui->showTranslatedStatusText(
					"Debug info, profiler graph, and wireframe hidden");
		} else {
...
#endif
		v2s32 center(
				driver->getScreenSize().Width / 2, driver->getScreenSize().Height / 2);
...
void Game::updatePlayerControl(const CameraOrientation &cam)
{
	...
	PlayerControl control(input->isKeyDown(KeyType::FORWARD),
			input->isKeyDown(KeyType::BACKWARD), input->isKeyDown(KeyType::LEFT),
			input->isKeyDown(KeyType::RIGHT), isKeyDown(KeyType::JUMP),
			isKeyDown(KeyType::SPECIAL1), isKeyDown(KeyType::SNEAK),
			isKeyDown(KeyType::ZOOM), input->getLeftState(), input->getRightState(),
			cam.camera_pitch, cam.camera_yaw,
...
void Game::handleClientEvent_ShowFormSpec(ClientEvent *event, CameraOrientation *cam)
{
	if (event->show_formspec.formspec->empty()) {
		auto formspec = m_game_ui->getFormspecGUI();
		if (formspec &&
				(event->show_formspec.formname->empty() ||
...
void Game::handleClientEvent_ShowLocalFormSpec(ClientEvent *event, CameraOrientation *cam)
{
	...
	GUIFormSpecMenu::create(m_game_ui->getFormspecGUI(), client, &input->joystick, fs_src,
...
void Game::handleClientEvent_SetSky(ClientEvent *event, CameraOrientation *cam)
{
	...
	if (event->set_sky->type == "regular") {
		...
		sky->setHorizonTint(event->set_sky->fog_sun_tint, event->set_sky->fog_moon_tint,
				event->set_sky->fog_tint_type);
	} else if (event->set_sky->type == "skybox" && event->set_sky->textures.size() == 6) {
		...
		sky->setHorizonTint(event->set_sky->bgcolor, event->set_sky->bgcolor, "custom");
	}
...
void Game::handleClientEvent_SetSun(ClientEvent *event, CameraOrientation *cam)
{
	...
	sky->setSunTexture(
			event->sun_params->texture, event->sun_params->tonemap, texture_src);
...
void Game::handleClientEvent_SetMoon(ClientEvent *event, CameraOrientation *cam)
{
	...
	sky->setMoonTexture(
			event->moon_params->texture, event->moon_params->tonemap, texture_src);
...
void Game::handleClientEvent_OverrideDayNigthRatio(
		ClientEvent *event, CameraOrientation *cam)
{
	client->getEnv().setDayNightRatioOverride(event->override_day_night_ratio.do_override,
...
void Game::updateCamera(u32 busy_time, f32 dtime)
{
	...
	*/
	...
	if (!m_flags.disable_camera_update) {
		client->getEnv().getClientMap().updateCamera(
				camera_position, camera_direction, camera_fov, camera_offset);
...
void Game::processPlayerInteraction(f32 dtime, bool show_hud, bool show_debug)
{
	...
	*/
	...
	PointedThing pointed = updatePointedThing(shootline, selected_def.liquids_pointable,
	...
	if (runData.digging) {
		if (input->getLeftReleased()) {
			...
			if (pointed.type == POINTEDTHING_NODE &&
					...
					pointed.node_undersurface == runData.pointed_old.node_undersurface) {
		...
		bool liquids_pointable, bool look_for_object, const v3s16 &camera_offset)
{
	...
	if (result.type == POINTEDTHING_OBJECT) {
		...
		n.getSelectionBoxes(
				nodedef, &boxes, n.getNeighbors(result.node_undersurface, &map));

		f32 d = 0.002 * BS;
		for (std::vector<aabb3f>::const_iterator i = boxes.begin(); i != boxes.end();
				++i) {
			aabb3f box = *i;
		...
		hud->setSelectedFaceNormal(v3f(result.intersection_normal.X,
				result.intersection_normal.Y, result.intersection_normal.Z));
	}
	...
	*/
	...
	if (meta) {
		m_game_ui->setInfoText(
				unescape_translate(utf8_to_wide(meta->getString("infotext"))));
	} else {
		MapNode n = map.getNode(nodepos);

		if (nodedef_manager->get(n).tiledef[0].name == "unknown_node.png") {
			m_game_ui->setInfoText(
					L"Unknown node: " + utf8_to_wide(nodedef_manager->get(n).name));
		}
	}
	...
	if ((input->getRightClicked() ||
		...
		bool placed =
				nodePlacement(def, selected_item, nodepos, neighbourpos, pointed, meta);
		...
		const PointedThing &pointed, const NodeMetadata *meta)
{
	...
	if (meta && !meta->getString("formspec").empty() && !random_input &&
		...
		NodeMetadataFormSource *fs_src =
				new NodeMetadataFormSource(&client->getEnv().getClientMap(), nodepos);
	...
	if (prediction.empty() ||
			(nodedef->get(node).rightclickable && !isKeyDown(KeyType::SNEAK))) {
		// Report to server
		client->interact(INTERACT_PLACE, pointed);
		return false;
	}
	...
	if (!found) {
		errorstream << "Node placement prediction failed for " << selected_def.name
					<< " (places " << prediction << ") - Name not known" << std::endl;
	...
	if (predicted_f.param_type_2 == CPT2_FACEDIR ||
		...
		v3s16 dir = nodepos -
				floatToInt(client->getEnv().getLocalPlayer()->getPosition(), BS);
		...
		const v3f &player_position, bool show_debug)
{
	std::wstring infotext =
			unescape_translate(utf8_to_wide(runData.selected_object->infoText()));
...
void Game::updateFrame(
		ProfilerGraph *graph, RunStats *stats, f32 dtime, const CameraOrientation &cam)
{
	...
	*/
	...
	if (m_cache_enable_noclip && m_cache_enable_free_move) {
		...
		direct_brightness =
				client->getEnv().getClientMap().getBackgroundBrightness(
						MYMIN(runData.fog_range * 1.2, 60 * BS), daynight_ratio,
						(int)(old_brightness * 255.5), &sunlight_seen) /
				255.0;
	}
	...
	if (time_of_day_smooth > 0.8 && time_of_day < 0.2)
		time_of_day_smooth =
				time_of_day_smooth * (1.0 - todsm) + (time_of_day + 1.0) * todsm;
	else
	...
	if (clouds) {
		if (sky->getCloudsVisible()) {
			...
			if (clouds->isCameraInsideCloud() && m_cache_enable_fog) {
				...
				// color(s)
				video::SColor clouds_dark = clouds->getColor().getInterpolated(
	...
	*/
	...
	if (m_cache_enable_fog) {
		driver->setFog(sky->getBgColor(), video::EFT_FOG_LINEAR,
				runData.fog_range * m_cache_fog_start, runData.fog_range * 1.0, 0.01,
				false, // pixel fog
				true // range fog
		);
	} else {
		driver->setFog(
				sky->getBgColor(), video::EFT_FOG_LINEAR, 100000 * BS, 110000 * BS, 0.01f,
				false, // pixel fog
				false // range fog
		);
	}
	...
	*/
	...
	if (runData.update_draw_list_timer >= 0.2 ||
			runData.update_draw_list_last_cam_dir.getDistanceFrom(camera_direction) >
					0.2 ||
	...
	*/
	if (runData.damage_flash > 0.0f) {
		...
		driver->draw2DRectangle(
				color, core::rect<s32>(0, 0, screensize.X, screensize.Y), NULL);
...
void Game::showPauseMenu()
{
	...
	porting::mt_snprintf(control_text_buf, sizeof(control_text_buf),
			...
			GET_KEY_NAME(keymap_inventory), GET_KEY_NAME(keymap_chat));
...
void the_game(bool *kill, bool random_input, InputHandler *input,
		...
		std::string &error_message, ChatBackend &chat_backend, bool *reconnect_requested,
		const SubgameSpec &gamespec, // Used for local game
		bool simple_singleplayer_mode)
{
	Game game;
	...
	try {
		if (game.startup(kill, random_input, input, map_dir, playername, password,
					&server_address, port, error_message, reconnect_requested,
					&chat_backend, gamespec, simple_singleplayer_mode)) {
			game.run();
		}
