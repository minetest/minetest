...
void Clouds::render()
{
	...
	*/
	...
	// Set our own fog
	driver->setFog(fog_color, fog_type, cloud_full_radius * 0.5, cloud_full_radius * 1.2,
			fog_density, fog_pixelfog, fog_rangefog);
...
#define INAREA(x, z, radius)                                                             \
	...
	for (s16 zi0 = -m_cloud_radius_i; zi0 < m_cloud_radius_i; zi0++)
		...
		if(xi <= 0)
			...
			video::S3DVertex v[4] = { video::S3DVertex(0, 0, 0, 0, 0, 0, c_top, 0, 1),
			...
			for (int i = 0; i < num_faces_to_draw; i++) {
				...
				driver->drawVertexPrimitiveList(v, 4, indices, 2, video::EVT_STANDARD,
...
void Clouds::update(const v3f &camera_p, const video::SColorf &color_diffuse)
{
	...
	if (m_enable_3d) {
		...
		if (camera_height >= m_box.MinEdge.Y && camera_height <= m_box.MaxEdge.Y) {
			v2f camera_in_noise;
...
bool Clouds::gridFilled(int x, int y) const
{
	...
	float noise = noise2d_perlin(
			(float)x * cloud_size_noise, (float)y * cloud_size_noise, m_seed, 3, 0.5);
