...
ClientActiveObject *GenericCAO::getParent() const
{
	...
	if ((attached_id != 0) && (attached_id != getId())) {
...
{
	// Should be true when removing the object permanently and false when refreshing
	// (eg: updating visuals)
	...
	if (m_prop.visual == "sprite") {
		...
		if (mesh) {
			...
			setMeshColor(m_animated_meshnode->getMesh(),
					video::SColor(255, li, li, li));

			setAnimatedMeshColor(m_animated_meshnode,
					video::SColor(255, li, li, li));

			m_animated_meshnode->setMaterialFlag(video::EMF_LIGHTING, true);
			m_animated_meshnode->setMaterialFlag(
					video::EMF_BILINEAR_FILTER, false);
...
void GenericCAO::updateBonePosition()
{
	...
	m_animated_meshnode->setJointMode(
			irr::scene::EJUOR_CONTROL); // To write positions to the mesh on
						    // render
	for (std::unordered_map<std::string, core::vector2d<v3f>>::const_iterator ii =
					m_bone_position.begin();
...
void GenericCAO::processMessage(const std::string &data)
{
	...
	if (cmd == GENERIC_CMD_SET_PROPERTIES) {
		...
		// immediatly reset a engine issued texture modifier if a mod sends a
		// different one
		if (m_reset_textures_timer > 0) {
			m_reset_textures_timer = -1;
			updateTextures(m_previous_texture_modifier);
		...
		if (parent_id != old_parent_id) {
			if (GenericCAO *old_parent = m_env->getGenericCAO(
					    old_parent_id)) {
		...
		if (!m_is_local_player) {
			m_attached_to_local = getParent() != NULL &&
					      getParent()->isLocalPlayer();
			// Objects attached to the local player should be hidden by
			// default
