...
public:
	MainShaderConstantSetter() : m_world_view_proj("mWorldViewProj"), m_world("mWorld") {}
			...
			video::IMaterialRendererServices *services, bool is_highlevel)
	{
		...
		if (is_highlevel)
			m_world_view_proj.set(
					*reinterpret_cast<float(*)[16]>(worldViewProj.pointer()), services);
		else
...
void ShaderSource::rebuildShaders()
{
	...
	*/
	...
	for (ShaderInfo &i : m_shaderinfo_cache) {
		ShaderInfo *info = &i;
		if (!info->name.empty()) {
			*info = generate_shader(info->name, info->material_type, info->drawtype,
		...
		SourceShaderCache *sourcecache)
{
	...
	if (!vertex_program.empty() && !driver->queryFeature(video::EVDF_VERTEX_SHADER_1_1) &&
	...
	if (!pixel_program.empty() && !driver->queryFeature(video::EVDF_PIXEL_SHADER_1_1) &&
	...
	if (!geometry_program.empty() && !driver->queryFeature(video::EVDF_GEOMETRY_SHADER)) {
	...
	if (is_highlevel) {
		...
		shadermat = gpu->addShaderMaterial(vertex_program_ptr, // Vertex shader program
				pixel_program_ptr, // Pixel shader program
				cb, // Set-constant callback
				shaderinfo.base_material, // Base material
