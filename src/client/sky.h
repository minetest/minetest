...
private:
	...
	{
		video::SColor result =
				video::SColor(col1.getAlpha() * (1 - factor) + col2.getAlpha() * factor,
	...
	{
		video::SColorf result = video::SColorf(col1.r * (1 - factor) + col2.r * factor,
	...
	void place_sky_body(std::array<video::S3DVertex, 4> &vertices, float horizon_position,
			float day_position);
	void setSkyDefaults();
};
