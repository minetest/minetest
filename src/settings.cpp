...
{
	if (value.substr(0, 3) == "\"\"\"" || value.find("\n\"\"\"") != std::string::npos) {
	...
	while (is.good() && !end_found) {
		...
		case SPE_GROUP:
			...
			if (it != m_settings.end() && it->second.is_group) {
				...
				was_modified |=
						it->second.group->updateConfigObject(is, os, "}", tab_depth + 1);
			} else if (it == m_settings.end()) {
				// Remove by skipping
				was_modified = true;
		...
		const std::string &name, const FlagDesc *flagdesc, u32 *flagmask) const
{
	...
	if (getDefaultNoEx(name, value)) {
		flags = std::isdigit(value[0]) ? stoi(value) :
...
bool Settings::getStruct(
		const std::string &name, const std::string &format, void *out, size_t olen) const
{
	std::string valstr;
...
bool Settings::setEntry(
		const std::string &name, const void *data, bool set_group, bool set_default)
{
...
bool Settings::setFlagStr(
		const std::string &name, u32 flags, const FlagDesc *flagdesc, u32 flagmask)
{
	if (!flagdesc) {
		if (!(flagdesc = getFlagDescFallback(name)))
			return false; // Not found
...
{
	for (SettingEntries::const_iterator it = m_settings.begin(); it != m_settings.end();
			++it)
		delete it->second.group;
	m_settings.clear();
...
{
	for (SettingEntries::const_iterator it = m_defaults.begin(); it != m_defaults.end();
			++it)
		delete it->second.group;
	m_defaults.clear();
}
