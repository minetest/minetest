	...
	while (is.good())
	{
		...
		switch (event)
		{
			...
			case SPE_END:
				return true;
			case SPE_GROUP:
			{ // TODO: Are we supposed to use curly braces to denote case blocks?
...
) const
{
	...
	if (flagmask)
	{
		// TODO: Say something about why the next line uses a pointer.
...
bool Settings::getNoiseParamsFromValue(
		const std::string &name,
		NoiseParams &np
) const
{
...
bool Settings::getNoiseParamsFromGroup(
	const std::string &name, 
	NoiseParams &np
) const
{
...
)
{
	...
	if (!set_group && !checkValueValid(*(const std::string *)data))
		return false;
	{ // TODO: Possible syntax error: This block opens with no conditional, after a single-line then-block.
		MutexAutoLock lock(m_mutex);
...
{
	if (!flagdesc) // TODO: Can this be combined with the nested-if using && operator?
	{
		if (!(flagdesc = getFlagDescFallback(name)))
		{
...
{
	for (
			SettingEntries::const_iterator it = m_settings.begin();
			it != m_settings.end(); 
			++it // TODO: Is this increment operator placed correctly?
	)
	{
		delete it -> second.group;
	}
...
{
	for (
			SettingEntries::const_iterator it = m_defaults.begin();
			it != m_defaults.end(); 
			++it // TODO: Is this increment operator placed correctly?
	)
	{
...
void Settings::doCallbacks(const std::string &name) const
{
	...
	if (it_cbks != m_callbacks.end())
	{
		...
		for (it = it_cbks->second.begin(); it != it_cbks->second.end(); ++it)
			(it -> first)(name, it->second); // TODO: Describe what this unusual syntax does.
	}
}
