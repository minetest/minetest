...
void ChatBuffer::addLine(const std::wstring &name, const std::wstring &text)
{
	...
	// TODO: Describe what happens when the number of rows is greater than zero.
	if (m_rows > 0)
	{
...
void ChatBuffer::deleteOldest(u32 count)
{
	...
	{
		++del_unformatted; // TODO: Is this increment operator placed correctly?
		// keep m_formatted in sync
		if (del_formatted < m_formatted.size())
		{

			sanity_check(m_formatted[del_formatted].first);
			++del_formatted; // TODO: Is this increment operator placed correctly?
			while (del_formatted < m_formatted.size() &&
					!m_formatted[del_formatted].first)
				++del_formatted; // TODO: Is this increment operator placed correctly?
		}
		--count; // TODO: Is this increment operator placed correctly?
	}
...
{
	u32 count = 0;
	while (count < m_unformatted.size() && 
			m_unformatted[count].age > maxAge)
	{
		++count; // TODO: Is this increment operator placed correctly?
	}
	deleteOldest(count);
...
void ChatBuffer::reformat(u32 cols, u32 rows)
{
	...
	else if (cols != m_cols || rows != m_rows)
	{
		...
		// TODO: Describe the behavior of the return type involved in the next line.
		bool at_bottom = (m_scroll == getBottomScrollPos());
		if (!at_bottom)
		{
...
void ChatBuffer::scrollAbsolute(s32 scroll)
{
	...
	// TODO: Describe why the next line feels like a new section of the block.
	m_scroll = scroll;
	if (m_scroll < top)
	{
		m_scroll = top;
		...
		std::vector<ChatFormattedLine>& destination) const
{
	...
	if (!line.name.empty())
	{
		temp_frag.text   = L"<";
		temp_frag.column = 0;
		//temp_frag.bold = 0; // TODO: Why is this line depreciated?
		next_frags.push_back(temp_frag);
		temp_frag.text   = line.name;
		temp_frag.column = 0;
		//temp_frag.bold = 1; // TODO: Why is this line depreciated?
		next_frags.push_back(temp_frag);
		temp_frag.text   = L"> ";
		temp_frag.column = 0;
		//temp_frag.bold = 0; // TODO: Why is this line depreciated?
	...
	//EnrichedString line_text(line.text); // TODO: Why is this line depreciated?
	...
	while (!next_frags.empty() || in_pos < line.text.size())
	{
		...
		while (!next_frags.empty())
		{
			...
			{
				// Fragment does not fit into current line, so split it up.
				temp_frag.text   = frag.text.substr(0, cols - out_column);
				temp_frag.column = out_column;
				//temp_frag.bold = frag.bold; // TODO: Why is this line deprecated?
			...
			if (out_column == cols || text_processing)
			{
				...
				// TODO: describe the purpose of the following line
		...
		if (in_pos < line.text.size())
		{
			...
			// on a word boundary.
			u32 // TODO: If declarations are to be combined why only these two?
					frag_length = 1, 
					space_pos   = 0;
					...
					frag_length < remaining_in_output)
			{
				...
				++frag_length; // TODO: Is this increment operator placed correctly?
			}
			if (space_pos != 0 && frag_length < remaining_in_input)
			{
				// Reminder: in C++ the increment operator assigns the new value.
				frag_length = space_pos + 1; 
			}
			temp_frag.text = line.text.substr(in_pos, frag_length);
			temp_frag.column = 0;
			//temp_frag.bold = 0; // TODO: Why is this line deprecated?
			next_frags.push_back(temp_frag);
			in_pos += frag_length;
...
{
	// TODO: Why is this class constructor an empty block?
}
...
void ChatPrompt::historyPrev()
{
	...
	{
		--m_history_index; // TODO: Is this decrement operator placed correctly?
...
void ChatPrompt::historyNext()
{
	...
	{
		++m_history_index; // TODO: Is this increment operator placed correctly?
		replace(m_history[m_history_index]);
	}
}
...
void ChatPrompt::nickCompletion(const std::list<std::string>& names, bool backwards)
{
	...
	if (initial)
	{
		...
		{
			--prefix_start; // TODO: Is this increment operator placed correctly?
		}
		while (prefix_end < m_line.size() && !iswspace(m_line[prefix_end]))
		{
			++prefix_end; // TODO: Is this increment operator placed correctly?
		}
	...
	if (!initial)
	{
		...
		{
			++word_end; // TODO: Is this incremenet operator placed correctly?
		}
	...
	if (word_end < m_line.size() && iswspace(m_line[word_end]))
		++word_end; // TODO: Is this increment operator placed correctly?
...
void ChatBackend::reformat(u32 cols, u32 rows)
{
	...
	// no need to reformat m_recent_buffer, its formatted lines are not used
...
void ChatBackend::step(float dtime)
{
	...
	m_recent_buffer.deleteByAge(60.0); // TODO: Why is this argument a constant literal?
...
{
	// TODO: Why is this not typecast as s32 here when in scrollPageUp() it is?
