...
MapBlock::~MapBlock()
{
// TODO: Say something about whether preprocessor code is never indented.
#ifndef SERVER
	{
		delete mesh;
		mesh = nullptr;
...
MapNode MapBlock::getNodeParent(v3s16 p, bool *is_valid_position)
{
	...
	if (!data)
	{
		...
		return {CONTENT_IGNORE}; // TODO: Why does this return-statement require a block?
	}
	if (is_valid_position)
	{
		*is_valid_position = true;
	}
	return data[p.Z * zstride + p.Y * ystride + p.X]; // TODO: Why does this return statement not require a block?
}

std::string MapBlock::getModifiedReasonString()
{
	std::string reason;
	const u32 ubound = MYMIN(
		sizeof(m_modified_reason) * CHAR_BIT,
...
void MapBlock::actuallyUpdateDayNightDiff()
{
	...
	// If some lighting values differ, check if the whole thing is	just air. 
	// If it is just air, differs = false
	if (differs)
	{
		bool only_air = true;
		for (u32 i = 0; i < nodecount; i++)
...
// sure we can handle all content ids. But it's absolutely worth it as it's a 
// speedup of 4 for one of the major time consuming functions on storing mapblocks.

static content_t getBlockNodeIdMapping_mapping[USHRT_MAX + 1];

static void getBlockNodeIdMapping( // TODO: Possible logic error: Why is this function not a method of an object class?
		NameIdMapping *nimap, 
		MapNode *nodes,
...
void MapBlock::deSerialize(std::istream &is, u8 version, bool disk)
{
	...
	try
	{
		...
		{
			content_nodemeta_deserialize_legacy(
					iss,
...
void MapBlock::deSerialize_pre22(std::istream &is, u8 version, bool disk)
{
	...
	else if (version <= 10)
	{
		...
		is.read((char *)&t8, 1);
		is_underground = t8;
		{ // TODO: Possible syntax error: Block begins with no preceding conditional.
	...
	for(u32 i=0; i < nodecount; i++)
	{
		...
		// Mineral
		if (
				nodedef -> getId("default:stone") == data[i].getContent()
				&& data[i].getParam1() == 1
		)
		{
...
std::string analyze_block(MapBlock *block)
{
	...
	else
	{
		...
		bool full_air  = true;
		bool some_air  = false;
		// Nested for-statements iterating across dimensions is allowed without indends:
