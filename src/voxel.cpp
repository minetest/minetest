...
void VoxelManipulator::addArea(const VoxelArea &area)
{
	...
	*/
	m_data  = new_data;
	m_flags = new_flags;
	// TODO: Describe what these lines do.
	delete[] old_data;
	delete[] old_flags;
	// dstream << "addArea done" << std::endl;
}

void VoxelManipulator::copyFrom(
		MapNode *src, 
 ...
 * inner loop and calculate the next index as we're iterating to gain performance.
 *
 ...
 * and for illustrative purposes). For example:
 *
 * src       <-------------------->|'''''' dest mod ''''''''
 * dest      <--------------------------------------------->
 *
 ...
 *
 * This method falls under the category "linear array and incrementing index".
 */
{
	s32 src_step  = src_area.getExtent().X;
	s32 dest_step = m_area.getExtent().X;
	s32 dest_mod  = m_area.index(
			to_pos.X, to_pos.Y, to_pos.Z + 1)
...
// Algorithms
{
	...
	/* 
	dstream 
		<< "clearFlag clearing area of size "
		<< "" << s.X << "x" << s.Y << "x" << s.Z << ""
		<< std::endl;
	*/
	// s32 count = 0;
	...
	*/
	...
	dstream
		<< "clearFlag changed " << count << " flags out of "
