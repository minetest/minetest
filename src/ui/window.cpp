...
namespace ui
{
	...
	{
		if (dst.empty() || texture == nullptr || tint.getAlpha() == 0) {
			return;
		}
	...
	void Window::drawAll()
	{
		...
		// might have changed since the last frame, recompute stuff before
		// drawing: restyle all the boxes and then layout each element in the
		// entire screen from the root to the leaves.
		m_root_elem->restyleAll();

		RectF parent_rect(getScreenSize());
		m_root_elem->relayoutAll(parent_rect, parent_rect);
	...
	bool Window::processInput(const SDL_Event &event)
	{
		...
		case SDL_MOUSEWHEEL: {
			...
			if (sendHoveredInput(event) != nullptr) {
				return true;
			}
	...
	Elem *Window::sendHoveredInput(const SDL_Event &event)
	{
		...
		for (size_t i = m_ordered_elems.size(); i > 0; i--) {
			...
			if (elem->hasPointedBox() && elem->processInput(event)) {
				return elem;
			}
		}
	...
	{
		// If this element's already focused, we don't need to do anything.
		if (new_focused == m_focused_elem) {
	...
	void Window::updateHoveredElem()
	{
		...
		if (isFocused()) {
			new_hovered = sendHoveredInput(notice);
		}
	...
	bool Window::isPointerOutside() const
	{
		...
		// have the noclip property set.
		for (Elem *elem : m_ordered_elems) {
			if (elem->hasPointedBox()) {
				return false;
			}
		}
			...
			std::unordered_map<Elem *, std::string> &elem_contents)
	{
		...
		for (size_t i = 0; i < num_elems; i++) {
			...
			// a non-existent element in getElem(). If the string has non-ID
