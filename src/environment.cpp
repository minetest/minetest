...
*/
inline static bool isPointableNode(
		const MapNode &n, const NodeDefManager *nodedef, bool liquids_pointable)
{
	const ContentFeatures &features = nodedef->get(n);
	return features.pointable || (liquids_pointable && features.isLiquid());
}
...
void Environment::continueRaycast(RaycastState *state, PointedThing *result)
{
	...
	while (state->m_iterator.m_current_index <= lastIndex) {
		...
		v3s16 delta = state->m_iterator.m_current_node_pos - state->m_previous_node;
		if (delta.X > 0) {
			...
			for (s16 y = new_nodes.MinEdge.Y; y <= new_nodes.MaxEdge.Y; y++)
				for (s16 z = new_nodes.MinEdge.Z; z <= new_nodes.MaxEdge.Z; z++) {
					...
					if (!(is_valid_position &&
								isPointableNode(
										n, nodedef, state->m_liquids_pointable))) {
						continue;
					}
					...
					n.getSelectionBoxes(nodedef, &boxes, n.getNeighbors(np, &map));
					...
					for (aabb3f &box : boxes) {
						...
						f32 distanceSq = (intersection_point - state->m_shootline.start)
												 .getLengthSQ();
