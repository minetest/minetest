	...
	if (m_sunlight_seen) {
		...
		if (m_overlay_visible) {
			...
			*/
			...
			// Render sides,
			// We use 7 because that's the texture material
			// ID we recieve from Lua and the event system.
			for (int i = 7; i < 11; i++){
		...
		if (m_stars_visible) {
			...
			do {
				...
				video::S3DVertex *vertices = new video::
					S3DVertex[m_star_count * 3];
