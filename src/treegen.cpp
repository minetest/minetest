		...
		TreeDef tree_definition)
{
	...
	// chance of inserting rules
	double prop_a = 10; //this is set to 1/1 propability, may change in the future
	double prop_b = 10;
	...
	double angle_in_radians = (double)tree_definition.angle*PI/180;
	double angleOffset_in_radians = (s16)(myrand_range(0,1)%MAX_ANGLE_OFFSET)*PI/180;
	
	//initialize rotation matrix, position and stacks for branches
	core::matrix4 rotation;
	rotation=setRotationAxisRadians(rotation, PI/2,v3f(0,0,1));
	...
	for(s16 i=0; i<iterations; i++)
	{
			...
			for(s16 j=0; j<(s16)axiom.size(); j++)
			{
				...
				{
				case 'A':
					if (prop_a >= myrand_range(0,9))
						temp+=tree_definition.rules_a;
					break;
				case 'B':
					if (prop_b >= myrand_range(0,9))
