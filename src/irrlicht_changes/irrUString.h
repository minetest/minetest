...
{
	return ((c >> 24) & 0x000000FF) | ((c >> 8) & 0x0000FF00) | ((c << 8) & 0x00FF0000) |
			((c << 24) & 0xFF000000);
}
...
public:
	...
	ustring16(ustring16<TAlloc> &&other)
		: array(other.array), encoding(other.encoding), allocated(other.allocated),
		  used(other.used)
	{
	...
	//! Inequality operator
	bool operator!=(const ustring16<TAlloc> &other) const { return !(*this == other); }
	...
	ustring16<TAlloc> &append(const uchar8_t *const other, u32 length = 0xffffffff)
	{
		...
		if (len == unicode::BOM_ENCODE_UTF8_LEN) {
			if (memcmp(other, unicode::BOM_ENCODE_UTF8, unicode::BOM_ENCODE_UTF8_LEN) ==
					0)
		...
		for (u32 l = 0; l < len;) {
			...
			if (((c2[l] >> 6) & 0x03) == 0x02) { // Invalid continuation byte.
				...
				++l;
					...
					c2[l] == 0xC1) { // Invalid byte - overlong encoding.
				array[pos++] = unicode::UTF_REPLACEMENT_CHARACTER;
				++l;
			} else if ((c2[l] & 0xF8) == 0xF0) { // 4 bytes UTF-8, 2 bytes UTF-16.
			...
			} else if ((c2[l] & 0xF0) == 0xE0) { // 3 bytes UTF-8, 1 byte UTF-16.
				// Check for a full string.
				if ((l + 2) >= len) {
					array[pos++] = unicode::UTF_REPLACEMENT_CHARACTER;
					l += 2;
				...
				array[pos++] = ch;
				l += 3;
			} else if ((c2[l] & 0xE0) == 0xC0) { // 2 bytes UTF-8, 1 byte UTF-16.
				// Check for a full string.
				if ((l + 1) >= len) {
					array[pos++] = unicode::UTF_REPLACEMENT_CHARACTER;
					l += 1;
				...
				// Validate.
				if (c2[l] >
						0x7F) { // Values above 0xF4 are restricted and aren't used.  By now, anything above 0x7F is invalid.
					array[pos++] = unicode::UTF_REPLACEMENT_CHARACTER;
				} else
	...
	ustring16<TAlloc> &append(const uchar16_t *const other, u32 length = 0xffffffff)
	{
		...
		if (memcmp(other, unicode::BOM_ENCODE_UTF16_LE, unicode::BOM_ENCODE_UTF16_LEN) ==
				0)
	...
	ustring16<TAlloc> &append(const uchar32_t *const other, u32 length = 0xffffffff)
	{
		...
		// Check for the BOM to determine the string's endianness.
		unicode::EUTF_ENDIAN c_end = unicode::EUTFEE_NATIVE;
		if (memcmp(other, unicode::BOM_ENCODE_UTF32_LE, unicode::BOM_ENCODE_UTF32_LEN) ==
				0)
			c_end = unicode::EUTFEE_LITTLE;
	...
	//! \return A reference to our current string.
	ustring16<TAlloc> &replace(
			const ustring16<TAlloc> &toReplace, const ustring16<TAlloc> &replaceWith)
	{
		if (toReplace.size() == 0)
			return *this;
	...
	ustring16<TAlloc> &remove(uchar32_t c)
	{
		...
		u32 len = (c > 0xFFFF ?
						2 :
						1); // Remove characters equal to the size of c as a UTF-16 character.
	...
	ustring16<TAlloc> &trim(const ustring16<TAlloc> &whitespace = " \t\n\r")
	{
		...
		const s32 begin =
				findFirstCharNotInList(utf32white.const_pointer(), whitespace.used + 1);
		if (begin == -1)
			return (*this = "");

		const s32 end =
				findLastCharNotInList(utf32white.const_pointer(), whitespace.used + 1);
	...
	*/
			...
			bool ignoreEmptyTokens = true, bool keepSeparators = false) const
	{
		...
		while (!i.atEnd()) {
			...
			for (u32 j = 0; j < count; ++j) {
				if (ch == c[j]) {
					if ((!ignoreEmptyTokens || pos - lastpos != 0) && !lastWasSeparator)
						ret.push_back(
								ustring16<TAlloc>(&array[lastpospos], pos - lastpos));
					foundSeparator = true;
	...
	*/
	template <class container>
	u32 split(container &ret, const ustring16<TAlloc> &c, bool ignoreEmptyTokens = true,
	...
	{
		if (encoding == unicode::EUTFE_UTF16_LE || encoding == unicode::EUTFE_UTF32_LE)
...
inline ustring16<TAlloc> operator+(
		const ustring16<TAlloc> &left, const std::basic_string<B, A, BAlloc> &right)
{
	ustring16<TAlloc> ret(left);
	ret += right;
	return ret;
...
template <typename TAlloc, typename B, typename A, typename BAlloc>
inline ustring16<TAlloc> operator+(
		const std::basic_string<B, A, BAlloc> &left, const ustring16<TAlloc> &right)
{
	ustring16<TAlloc> ret(left);
	ret += right;
	return ret;
