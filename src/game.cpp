...
)
{
	...
	*/
	...
	draw_load_screen(wgettext("Loading..."), device, font,0,0);
	...
	*/
	...
	if(address == ""){
		draw_load_screen(wgettext("Creating server..."), device, font,0,25);
	...
	*/
	...
	draw_load_screen(wgettext("Creating client..."), device, font,0,50);
	...
	draw_load_screen(wgettext("Resolving address..."), device, font,0,75);
	...
	*/
	...
	try{
		...
		while(device->run())
		{
			...
			// Display status
			draw_load_screen(wgettext("Connecting to server..."), device, font, frametime, 100);
