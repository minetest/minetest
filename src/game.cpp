		...
		v3s16 nodepos, v3s16 neighbourpos)
{
	...
	if(prediction != "" && !nodedef->get(map.getNode(nodepos)).rightclickable)
	{
		...
		try{
			...
			if (!nodedef->get(n).walkable || (client.checkPrivilege("noclip") && g_settings->getBool("noclip")) ||
				(nodedef->get(n).walkable && neighbourpos != player->getStandingNodePos() + v3s16(0,1,0) &&
			...
			*/
					...
					client.checkPrivilege("interact"))
			{
				...
				else
				{
					...
					if (playeritem_def.node_placement_prediction == "" || nodedef->get(map.getNode(nodepos)).rightclickable)
