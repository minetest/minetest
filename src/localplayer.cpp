		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	bool noclip = m_client->checkLocalPrivilege("noclip") &&
		builtin_settings.noclip;
	bool free_move = builtin_settings.free_move && fly_allowed;
...
void LocalPlayer::applyControl(float dtime, Environment *env)
{
	...
	bool fast_move = fast_allowed && builtin_settings.fast_move;
	// When aux1_descends is enabled the fast key is used to go down, so fast isn't possible
	bool fast_climb = fast_move && control.aux1 && !builtin_settings.aux1_descends;
	bool continuous_forward = builtin_settings.continuous_forward;
	bool always_fly_fast = builtin_settings.always_fly_fast;
	...
	// Old descend control
	if(builtin_settings.aux1_descends)
	{
	...
	{
		if (free_move) {
			if (builtin_settings.aux1_descends || always_fly_fast) {
				if (fast_move)
					speedV.Y = movement_speed_fast;
				else
					speedV.Y = movement_speed_walk;
		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	bool noclip = m_client->checkLocalPrivilege("noclip") &&
		builtin_settings.noclip;
	bool free_move = noclip && fly_allowed && builtin_settings.free_move;
	if (free_move) {
		position += m_speed * dtime;
		setPosition(position);
		m_sneak_node_exists = false;
	...
	*/
	if (control.sneak && m_sneak_node_exists &&
			!(fly_allowed && builtin_settings.free_move) && !in_liquid &&
	...
	*/
	...
	if (collision_info && !(builtin_settings.free_move && fly_allowed)) {
