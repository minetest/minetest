...
{
	settings->setDefault(
			"mgcarpathian_spflags", flagdesc_mapgen_carpathian, MGCARPATHIAN_CAVERNS);
}
...
int MapgenCarpathian::getSpawnLevelAtPoint(v2s16 p)
{
	...
	if (spflags & MGCARPATHIAN_RIVERS) {
		float river =
				std::fabs(NoisePerlin2D(&noise_rivers->np, p.X, p.Y, seed)) - river_width;
		if (river < 0.0f)
	...
	for (s16 y = water_level; y <= water_level + 32; y++) {
		...
		if ((spflags & MGCARPATHIAN_RIVERS) && river <= valley_width) {
			if (valley < 0.0f) {
				// River channel
				surface_level = std::fmin(
						surface_level, water_level - std::sqrt(-valley) * river_depth);
			...
			for (s16 y = node_min.Y - 1; y <= node_max.Y + 1;
				...
				if ((spflags & MGCARPATHIAN_RIVERS) && node_max.Y >= water_level - 16 &&
					...
					if (valley < 0.0f) {
						...
						// Valley slopes
						surface_level =
								water_level + (surface_level - water_level) * valley;
					}
				}
