	...
	gennotify(emerge->gen_notify_on, emerge->gen_notify_on_deco_ids)
{
	...
	flags        = params->flags;
	csize        = v3POS(1, 1, 1) * (params->chunksize * MAP_BLOCKSIZE);
...
u32 Mapgen::getBlockSeed(v3POS p, s32 seed)
{
	return (u32)seed   +
		p.Z * 38134234 +
		p.Y * 42123    +
...
u32 Mapgen::getBlockSeed2(v3POS p, s32 seed)
{
...
POS Mapgen::findLiquidSurface(v2POS p2d, POS ymin, POS ymax)
{
	const v3POS &em = vm->m_area.getExtent();
	u32 i = vm->m_area.index(p2d.X, ymax, p2d.Y);
	POS y;
...
void Mapgen::getSurfaces(v2POS p2d, POS ymin, POS ymax,
...
inline bool Mapgen::isLiquidHorizontallyFlowable(u32 vi, v3POS em)
{
	u32 vi_neg_x = vi;
...
void Mapgen::updateLiquid(UniqueQueue<v3POS> *trans_liquid, v3POS nmin, v3POS nmax)
{
	...
	for (POS z = nmin.Z + 1; z <= nmax.Z - 1; z++)
	for (POS x = nmin.X + 1; x <= nmax.X - 1; x++) {
		wasignored = true;
		wasliquid = false;
		waschecked = false;
		waspushed = false;
		...
		for (POS y = nmax.Y; y >= nmin.Y; y--) {
			...
			if (isignored || wasignored || isliquid == wasliquid) {
				...
				if (!waspushed && (ndef->get(vm->m_data[vi]).floodable ||
					...
					// node above this one
					trans_liquid->push_back(v3POS(x, y + 1, z));
				}
			}
...
void Mapgen::setLighting(u8 light, v3POS nmin, v3POS nmax)
{
...
void Mapgen::lightSpread(VoxelArea &a, std::queue<std::pair<v3POS, u8>> &queue,
...
void Mapgen::calcLighting(v3POS nmin, v3POS nmax, v3POS full_nmin, v3POS full_nmax,
...
void Mapgen::propagateSunlight(v3POS nmin, v3POS nmax, bool propagate_shadow)
{
...
void Mapgen::spreadLight(const v3POS &nmin, const v3POS &nmax)
{
	//TimeTaker t("spreadLight");
	std::queue<std::pair<v3POS, u8>> queue;
	VoxelArea a(nmin, nmax);
	...
	: Mapgen(mapgenid, params, emerge)
{
	...
	//// Allocate heightmap
	this->heightmap = new POS[csize.X * csize.Z];
...
void MapgenBasic::generateBiomes()
{
	...
	for (POS x = node_min.X; x <= node_max.X; x++, index++) {
		...
		POS biome_y_min = -MAX_MAP_GENERATION_LIMIT;
		...
		for (POS y = node_max.Y; y >= node_min.Y; y--) {
			...
			if (is_stone_surface || is_water_surface) {
				// (Re)calculate biome
				biome = biomegen->getBiomeAtIndex(index, v3POS(x, y, z));
...
void MapgenBasic::dustTopNodes()
{
	...
	for (POS x = node_min.X; x <= node_max.X; x++, index++) {
		...
		for (POS y = y_start; y >= node_min.Y - 1; y--) {
...
void MapgenBasic::generateCavesNoiseIntersection(POS max_stone_y)
{
...
void MapgenBasic::generateCavesRandomWalk(POS max_stone_y, POS large_cave_ymax)
{
...
bool MapgenBasic::generateCavernsNoise(POS max_stone_y)
{
...
void MapgenBasic::generateDungeons(POS max_stone_y)
{
	...
	dp.room_size_min       = v3POS(5, 5, 5);
	dp.room_size_max       = v3POS(12, 6, 12);
	dp.room_size_large_min = v3POS(12, 6, 12);
	...
	dp.holesize            = v3POS(holewidth, 3, holewidth);
	dp.corridor_len_min    = 1;
	dp.corridor_len_max    = 13;

	// Get biome at mapchunk midpoint
	v3POS chunk_mid = node_min + (node_max - node_min) / v3POS(2, 2, 2);
...
bool GenerateNotifier::addEvent(GenNotifyType type, v3POS pos, u32 id)
{
...
void GenerateNotifier::getEvents(
	std::map<std::string, std::vector<v3POS> > &event_map)
{
...
void MapgenParams::calcMapgenEdges()
{
	...
	// Uses same calculation as ServerMap::blockpos_over_mapgen_limit(v3POS p)
	POS mapgen_limit_b = rangelim(mapgen_limit,
		0, MAX_MAP_GENERATION_LIMIT) / MAP_BLOCKSIZE;
	// Effective mapgen limits, in nodes
	POS mapgen_limit_min = -mapgen_limit_b * MAP_BLOCKSIZE;
	POS mapgen_limit_max = (mapgen_limit_b + 1) * MAP_BLOCKSIZE - 1;
	// Number of complete chunks from central chunk fullminp/fullmaxp
	// to effective mapgen limits.
	POS numcmin = MYMAX((ccfmin - mapgen_limit_min) / csize_n, 0);
	POS numcmax = MYMAX((mapgen_limit_max - ccfmax) / csize_n, 0);
