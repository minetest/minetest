...
FlagDesc flagdesc_mapgen_v7[] = { { "mountains", MGV7_MOUNTAINS },
	{ "ridges", MGV7_RIDGES }, { "floatlands", MGV7_FLOATLANDS },
	{ "caverns", MGV7_CAVERNS }, { NULL, 0 } };
...
int MapgenV7::getSpawnLevelAtPoint(v2s16 p)
{
	...
	if (spflags & MGV7_RIDGES) {
		float width	  = 0.2f;
		float uwatern = NoisePerlin2D(&noise_ridge_uwater->np, p.X, p.Y, seed) * 2.0f;
	...
	s16 max_spawn_y = std::fmax(
			std::fmax(noise_terrain_alt->np.offset, noise_terrain_base->np.offset),
			water_level + 16);
...
void MapgenV7::makeChunk(BlockMakeData *data)
{
	...
	if (flags & MG_LIGHT)
		calcLighting(node_min - v3s16(0, 1, 0), node_max + v3s16(0, 1, 0), full_node_min,
				full_node_max, propagate_shadow);
...
{
	float mnt_h_n = std::fmax(NoisePerlin2D(&noise_mount_height->np, x, z, seed), 1.0f);
	...
	for (s16 z = node_min.Z; z <= node_max.Z; z++)
		for (s16 x = node_min.X; x <= node_max.X; x++, index2d++) {
			...
			for (s16 y = node_min.Y - 1; y <= node_max.Y + 1;
					y++, index3d += ystride, VoxelArea::add_y(em, vi, 1), cache_index++) {
	...
	for (s16 z = node_min.Z; z <= node_max.Z; z++)
		for (s16 y = node_min.Y - 1; y <= node_max.Y + 1; y++) {
			...
			for (s16 x = node_min.X; x <= node_max.X; x++, index3d++, vi++) {
				...
				float nridge =
						noise_ridge->result[index3d] * std::fmax(altitude, 0.0f) / 7.0f;
