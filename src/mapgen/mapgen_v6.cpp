...
FlagDesc flagdesc_mapgen_v6[] = { { "jungles", MGV6_JUNGLES },
	...
	{ "snowbiomes", MGV6_SNOWBIOMES }, { "flat", MGV6_FLAT }, { "trees", MGV6_TREES },
	{ NULL, 0 } };
	...
	: Mapgen(MAPGEN_V6, params, emerge)
{
	...
	noise_humidity = new Noise(&params->np_humidity, seed, csize.X + 2 * MAP_BLOCKSIZE,
			csize.Y + 2 * MAP_BLOCKSIZE);
	...
	if (c_dirt_with_grass == CONTENT_IGNORE)
		errorstream << "Mapgen v6: Mapgen alias 'mapgen_dirt_with_grass' is invalid!"
					<< std::endl;
...
float MapgenV6::baseTerrainLevel(
		float terrain_base, float terrain_higher, float steepness, float height_select)
{
...
float MapgenV6::baseTerrainLevelFromNoise(v2s16 p)
{
	...
	float terrain_base =
			NoisePerlin2D_PO(&noise_terrain_base->np, p.X, 0.5, p.Y, 0.5, seed);
	float terrain_higher =
			NoisePerlin2D_PO(&noise_terrain_higher->np, p.X, 0.5, p.Y, 0.5, seed);
	float steepness = NoisePerlin2D_PO(&noise_steepness->np, p.X, 0.5, p.Y, 0.5, seed);
	float height_select =
			NoisePerlin2D_PO(&noise_height_select->np, p.X, 0.5, p.Y, 0.5, seed);
			...
			for (s16 y = node_min.Y; y <= node_max.Y; y++) {
				if (vm->m_data[i].getContent() == CONTENT_IGNORE) {
					if (y <= surface_y) {
						...
						vm->m_data[i] = (y >= MGV6_ICE_BASE && bt == BT_TUNDRA) ?
								n_ice :
								n_water_source;
					} else {
						vm->m_data[i] = n_air;
...
void MapgenV6::moveMud(
		u32 remove_index, u32 place_index, u32 above_remove_index, v2s16 pos, v3s16 em)
{
...
void MapgenV6::placeTreesAndJungleGrass()
{
	...
	for (s16 z0 = 0; z0 < div; z0++)
		for (s16 x0 = 0; x0 < div; x0++) {
			...
			v2s16 p2d_min(node_min.X + sidelen * x0, node_min.Z + sidelen * z0);
			...
			if (bt == BT_JUNGLE) {
				...
				for (u32 i = 0; i < grass_count; i++) {
					...
					int mapindex =
							central_area_size.X * (z - node_min.Z) + (x - node_min.X);
			...
			for (u32 i = 0; i < tree_count; i++) {
				...
				int mapindex = central_area_size.X * (z - node_min.Z) + (x - node_min.X);
				s16 y = heightmap[mapindex];
				...
				// Trees grow only on mud and grass
				{
					...
					content_t c = vm->m_data[i].getContent();
					if (c != c_dirt && c != c_dirt_with_grass && c != c_dirt_with_snow)
						continue;
				}
				p.Y++;
				...
				if (bt == BT_JUNGLE) {
					...
					bool is_apple_tree =
							(myrand_range(0, 3) == 0) && getHaveAppleTree(v2s16(x, z));
...
void MapgenV6::generateCaves(int max_stone_y)
{
	...
	int volume_nodes =
			(node_max.X - node_min.X + 1) * (node_max.Y - node_min.Y + 1) * MAP_BLOCKSIZE;
	...
	for (u32 i = 0; i < caves_count + bruises_count; i++) {
		...
		cave.makeCave(
				vm, node_min, node_max, &ps, &ps2, large_cave, max_stone_y, heightmap);
	}
}
