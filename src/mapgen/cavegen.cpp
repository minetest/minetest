		...
		MMVManip *vm, v3s16 nmin, v3s16 nmax, biome_t *biomemap)
{
	...
	for (s16 z = nmin.Z; z <= nmax.Z; z++)
		for (s16 x = nmin.X; x <= nmax.X; x++, index2d++) {
			...
			for (s16 y = nmax.Y; y >= nmin.Y - 1;
					y--, index3d -= m_ystride, VoxelArea::add_y(em, vi, -1)) {
				content_t c = vm->m_data[vi].getContent();

				if (c == CONTENT_AIR || c == biome->c_water_top || c == biome->c_water) {
					column_is_open		= true;
...
bool CavernsNoise::generateCaverns(MMVManip *vm, v3s16 nmin, v3s16 nmax)
{
	...
	for (s16 z = nmin.Z; z <= nmax.Z; z++)
		for (s16 x = nmin.X; x <= nmax.X; x++, index2d++) {
			...
			u32 index3d =
					(z - nmin.Z) * m_zstride_1d + m_csize.Y * m_ystride + (x - nmin.X);
		...
		bool is_large_cave, int max_stone_height, s16 *heightmap)
{
	...
	if (gennotify) {
		...
		GenNotifyType notifytype =
				large_cave ? GENNOTIFY_LARGECAVE_BEGIN : GENNOTIFY_CAVE_BEGIN;
...
void CavesRandomWalk::makeTunnel(bool dirswitch)
{
	...
	if (large_cave) {
		maxlen = v3s16(
				rs_part_max_length_rs, rs_part_max_length_rs / 2, rs_part_max_length_rs);
	} else {
...
void CavesRandomWalk::carveRoute(v3f vec, float f, bool randomize_xz)
{
	...
	if (flooded) {
		if (use_biome_liquid) {
			...
			float nval =
					NoisePerlin3D(np_caveliquids, startp.X, startp.Y, startp.Z, seed);
			liquidnode = (nval < 0.40f && node_max.Y < water_level - 256) ? lavanode :
																			waternode;
		}
	}
...
void CavesV6::makeCave(MMVManip *vm, v3s16 nmin, v3s16 nmax, PseudoRandom *ps,
		PseudoRandom *ps2, bool is_large_cave, int max_stone_height, s16 *heightmap)
{
	assert(vm);
	assert(ps);
	assert(ps2);
...
void CavesV6::makeTunnel(bool dirswitch)
{
	...
	bool tunnel_above_ground =
			p1.Y > getSurfaceFromHeightmap(p1) && p2.Y > getSurfaceFromHeightmap(p2);
