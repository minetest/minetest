...
class GUIStaticText : public IGUIStaticText
{
public:

	// GUIStaticText is translated by EnrichedString.
	// No need to use translate_string()
	GUIStaticText(const EnrichedString &text, bool border, IGUIEnvironment* environment,
		...
		bool fillBackground = false)
	{
		if (parent == NULL) {
			...
			//
			// Thus, to find the parent, we create a dummy GUIStaticText and ask
		...
		irr::gui::IGUIStaticText *result = new GUIStaticText(
...
inline void setStaticText(irr::gui::IGUIStaticText *static_text, const EnrichedString &text)
{
	...
	if (static_text->hasType(EGUIET_ENRICHED_STATIC_TEXT)) {
		GUIStaticText* stext = static_cast<GUIStaticText*>(static_text);
		stext->setText(text);
	} else {
