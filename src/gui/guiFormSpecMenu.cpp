...
void GUIFormSpecMenu::parseList(parserData *data, const std::string &element)
{
		...
		((parts.size() > 5) && (m_formspec_version > FORMSPEC_API_VERSION)))
	{
		...
		v2s32 slot_size(
			slot_scale.X <= 0 ? imgsize.X : std::max<s32>(round(slot_scale.X * imgsize.X), 1),
			slot_scale.Y <= 0 ? imgsize.Y : std::max<s32>(round(slot_scale.Y * imgsize.Y), 1)
		);
		...
		if (data->real_coordinates) {
			...
			slot_spacing.X = slot_spacing.X < 0 ? spacing.X - imgsize.X :
					imgsize.X * slot_spacing.X;
			slot_spacing.Y = slot_spacing.Y < 0 ? spacing.Y - imgsize.Y :
					imgsize.Y * slot_spacing.Y;
		}

		slot_spacing.X = round(slot_spacing.X) + slot_size.X;
		slot_spacing.Y = round(slot_spacing.Y) + slot_size.Y;
