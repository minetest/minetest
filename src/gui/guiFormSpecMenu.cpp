...
void GUIFormSpecMenu::parseMesh(parserData *data, const std::string &element)
{
		...
		(parts.size() > 7 && m_formspec_version > FORMSPEC_API_VERSION))
	{
		...
		std::string inf_rotation = unescape_string(parts[5]);
		std::string mousectrl = unescape_string(parts[6]);
		...
		e->enableMouseControl(mousectrl != "false");

		if (vec_rot.size() >= 2)
			e->setRotation(v2f(stof(vec_rot[0]), stof(vec_rot[1])));

		e->enableContinuousRotation(is_yes(inf_rotation));
