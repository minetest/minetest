	...
	m_kill(kill)
{
	...
	if (g_settings->getBool("enable_sound") && g_sound_manager_singleton.get())
		m_sound_manager =
				createOpenALSoundManager(g_sound_manager_singleton.get(), &soundfetcher);
#endif
	if (!m_sound_manager)
	...
	m_irr_toplefttext = gui::StaticText::add(
			RenderingEngine::get_gui_env(), m_toplefttext, rect, false, true, 0, -1);
	...
	/* Create menu */
	m_menu =
			new GUIFormSpecMenu(joystick, m_parent, -1, m_menumanager, NULL /* &client */,
...
bool GUIEngine::loadMainMenuScript()
{
	...
	if (m_scriptdir.empty()) {
		m_scriptdir =
				porting::path_share + DIR_DELIM + "builtin" + DIR_DELIM + "mainmenu";
	}
...
void GUIEngine::run()
{
	...
	// Reset fog color
	{
		...
		driver->setFog(sky_color, fog_type, fog_start, fog_end, fog_density, fog_pixelfog,
				fog_rangefog);
	}
...
void GUIEngine::drawBackground(video::IVideoDriver *driver)
{
	...
	if (m_textures[TEX_LAYER_BACKGROUND].tile) {
		v2u32 tilesize(MYMAX(sourcesize.X, m_textures[TEX_LAYER_BACKGROUND].minsize),
	...
	draw2DImageFilterScaled(driver, texture,
			...
			core::rect<s32>(0, 0, sourcesize.X, sourcesize.Y), NULL, NULL, true);
}
