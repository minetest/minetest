...
void      read_inventory_list       (lua_State *L, int tableindex, Inventory *inv,
		const char *name, IGameDef *gdef, int forcesize=-1);
...
void                   push_flags_string (lua_State *L, FlagDesc *flagdesc, u32 flags,
		u32 flagmask);

u32                    read_flags_table  (lua_State *L, int table, FlagDesc *flagdesc,
		u32 *flagmask);
...
bool read_hud_change            (lua_State *L, HudElementStat &stat, HudElement *elem,
		void **value);
