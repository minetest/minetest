		...
		bool write_required, bool *write_allowed)
{
	...
	if (client) {
		...
		std::string safe_path = fs::AbsolutePath(porting::path_share + DIR_DELIM + "builtin");
		std::cout << safe_path << "->" << abs_path << std::endl;
		return norel_path.compare(0, safe_path.length(), safe_path) == 0;
	}
