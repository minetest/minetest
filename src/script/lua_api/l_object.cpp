...
ObjectRef *ObjectRef::checkobject(lua_State *L, int narg)
{
	...
	ObjectRef *ud = (ObjectRef *)luaL_checkudata(L, narg, className);
	if (!ud)
...
LuaEntitySAO *ObjectRef::getluaobject(ObjectRef *ref, const ServerEnvironment &senv)
{
	ServerActiveObject *sao = getobject(ref, senv);
	if (sao == nullptr)
		return nullptr;
...
PlayerSAO *ObjectRef::getplayersao(ObjectRef *ref, const ServerEnvironment &senv)
{
	ServerActiveObject *sao = getobject(ref, senv);
	if (sao == nullptr)
		return nullptr;
...
RemotePlayer *ObjectRef::getplayer(ObjectRef *ref, const ServerEnvironment &senv)
{
	PlayerSAO *playersao = getplayersao(ref, senv);
...
int ObjectRef::l_remove(lua_State *L)
{
	...
	ServerActiveObject *sao = getobject(ref, getServer(L)->getEnv());
	if (sao == nullptr)
		return 0;
	if (sao->getType() == ACTIVEOBJECT_TYPE_PLAYER)
		return 0;
...
int ObjectRef::l_punch(lua_State *L)
{
	...
	const auto &senv = getServer(L)->getEnv();
	ServerActiveObject *sao = getobject(ref, senv);
	ServerActiveObject *puncher = getobject(puncher_ref, senv);
...
int ObjectRef::l_right_click(lua_State *L)
{
	...
	const auto &senv = getServer(L)->getEnv();
	ServerActiveObject *sao = getobject(ref, senv);
	ServerActiveObject *sao2 = getobject(ref2, senv);
...
int ObjectRef::l_set_local_animation(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref, getServer(L)->getEnv());
	if (player == nullptr)
		return 0;
...
int ObjectRef::l_set_attach(lua_State *L)
{
	...
	const auto &senv = getServer(L)->getEnv();
	ServerActiveObject *sao = getobject(ref, senv);
	ServerActiveObject *parent = getobject(parent_ref, senv);
...
int ObjectRef::l_set_velocity(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *sao = getluaobject(ref, getServer(L)->getEnv());
	if (sao == nullptr)
		return 0;
...
int ObjectRef::l_set_acceleration(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref, getServer(L)->getEnv());
...
int ObjectRef::l_get_look_dir(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO* playersao = getplayersao(ref, getServer(L)->getEnv());
	if (playersao == nullptr)
		return 0;
