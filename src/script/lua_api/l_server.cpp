...
int ModApiServer::l_get_player_ip(lua_State *L)
{
	...
	try
	{
		...
		std::string ip_str = addr.serializeHost();
...
#define ERET(code)                                                             \
	...
	lua_pushstring(L, addr.serializeHost().c_str());
	lua_setfield(L, table, "address");

	int f = addr.getFamily();
	lua_pushnumber(L, f == AF_INET ? 4 :
			f == AF_INET6 ? 6 :
			0);
	lua_setfield(L, table, "ip_version");

	lua_pushnumber(L, min_rtt);
	lua_setfield(L, table, "min_rtt");

	lua_pushnumber(L, max_rtt);
	lua_setfield(L, table, "max_rtt");

	lua_pushnumber(L, avg_rtt);
	lua_setfield(L, table, "avg_rtt");

	lua_pushnumber(L, min_jitter);
	lua_setfield(L, table, "min_jitter");

	lua_pushnumber(L, max_jitter);
	lua_setfield(L, table, "max_jitter");

	lua_pushnumber(L, avg_jitter);
	lua_setfield(L, table, "avg_jitter");

	lua_pushnumber(L, uptime);
	lua_setfield(L, table, "connection_uptime");

#ifndef NDEBUG
	lua_pushnumber(L, ser_vers);
	lua_setfield(L, table, "serialization_version");

	lua_pushnumber(L, prot_vers);
	lua_setfield(L, table, "protocol_version");

	lua_pushnumber(L, major);
	lua_setfield(L, table, "major");

	lua_pushnumber(L, minor);
	lua_setfield(L, table, "minor");

	lua_pushnumber(L, patch);
	lua_setfield(L, table, "patch");

	lua_pushstring(L, vers_string.c_str());
	lua_setfield(L, table, "version_string");
