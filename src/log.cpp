...
public:
	...
	void logRaw(LogLevel lev, const std::string &line)
	{
		...
		__android_log_print(g_level_to_android[lev], PROJECT_NAME_C, "%s", line.c_str());
	}
};
...
void FileLogOutput::setFile(const std::string &filename, s64 file_size_max)
{
	...
	if (is_too_large) {
		...
		actionstream << "The log file grew too big; it is moved to " << filename_secondary
					 << std::endl;
	...
	if (!m_stream.good())
		throw FileNotGoodException(
				"Failed to open log file " + filename + ": " + strerror(errno));
	m_stream << "\n\n"
				"-------------"
			 << std::endl
			 << "  Separator" << std::endl
