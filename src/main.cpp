...
void drawMenuFooter(video::IVideoDriver* driver, bool clouds) {
	...
	std::string path = getTexturePath((clouds) ? "menufooter_clouds.png" : "menufooter.png");
	if (path[0]) {
		video::ITexture *footertexture =
			driver->getTexture(path.c_str());

		if (footertexture) {
			//v2s32 footersize(600, (footertexture->getOriginalSize().Height*600)/
			//	footertexture->getOriginalSize().Width);
			...
			while(kill == false)
			{
				...
				if(skip_main_menu == false)
				{
					...
					if (g_settings->getBool("menu_clouds")) {
						...
						clouds = new Clouds(smgr->getRootSceneNode(), smgr, -1, rand());
