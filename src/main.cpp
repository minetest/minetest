...
void drawMenuFooter(video::IVideoDriver* driver) {
	...
	core::dimension2d<u32> screensize = driver->getScreenSize();
	if (getTexturePath("menufooter.png") != "") {
		video::ITexture *footertexture =
			driver->getTexture(getTexturePath("menufooter.png").c_str());

		if (footertexture) {
			//v2s32 footersize(600, (footertexture->getOriginalSize().Height*600)/footertexture->getOriginalSize().Width);
			f32 mult = (((f32)screensize.Width*0.6)) / ((f32)footertexture->getOriginalSize().Width);

			v2s32 footersize(((f32)footertexture->getOriginalSize().Width) * mult,
					((f32)footertexture->getOriginalSize().Height) * mult);

			// If the empty space between the bottom window border and the main menu are not big enough for the footer, don't draw it
			s32 free_space = (((s32)screensize.Height)-320)/2;
			if (free_space > footersize.Y) {
				core::rect<s32> rect(0,0,footersize.X,footersize.Y);
				rect += v2s32(screensize.Width/2,screensize.Height-10-footersize.Y);
...
void drawMenuHeader(video::IVideoDriver* driver) {
	...
	if (getTexturePath("menuheader.png") != "") {
		video::ITexture *splashtexture =
		driver->getTexture(getTexturePath("menuheader.png").c_str());

		if(splashtexture) {
			//v2s32 splashsize((splashtexture->getOriginalSize().Width*100)/splashtexture->getOriginalSize().Height, 80);

			f32 mult = (((f32)screensize.Width / 2)) / ((f32)splashtexture->getOriginalSize().Width);

			v2s32 splashsize(((f32)splashtexture->getOriginalSize().Width) * mult,
					((f32)splashtexture->getOriginalSize().Height) * mult);
			// If the empty space between the top window border and the main menu are not big enough for the splash, don't draw it

			//splashsize *= (((f32)screensize.Width/2))/((f32)splashtexture->getOriginalSize().Width);
			//errorstream << splashsize.X << " " << splashtexture->getOriginalSize().Width << " " << (((f32)screensize.Width/2))/((f32)splashtexture->getOriginalSize().Width) << " " << screensize.Width/2 << std::endl;

			s32 free_space = (((s32)screensize.Height)-320)/2;
			if (free_space > splashsize.Y) {
				core::rect<s32> splashrect(0, 0, splashsize.X, splashsize.Y);
				splashrect += v2s32((screensize.Width/2)-(splashsize.X/2), ((free_space/2)-splashsize.Y/2)+10);
			...
			*/
			while(kill == false)
			{
				...
				if(skip_main_menu == false)
				{
					...
					Clouds *clouds = NULL;
					if (getTexturePath("menusplash.png") != "" || getTexturePath("menubg.png") == "") {
						cloud_menu_background = true;
						float cloud_height = 1000;
						clouds = new Clouds(smgr->getRootSceneNode(), smgr, -1, cloud_height);
						clouds->update(v2f(0, 0), video::SColor(255,200,200,255)); // We need only one update

						// A camera to see the clouds
						scene::ICameraSceneNode* camera = smgr->addCameraSceneNode(0, v3f(0,0,0), v3f(-100,60,0));
					...
					while(device->run() && kill == false)
					{
						...
						if (cloud_menu_background) {
							clouds->step(dtime*4); // *4 otherwise the clouds would move very slowly
							clouds->render();
							smgr->drawAll();
