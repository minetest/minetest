...
Database_Redis::Database_Redis(Settings &conf)
{
	...
	if (conf.exists("redis_password")) {
		...
		redisReply *reply =
				static_cast<redisReply *>(redisCommand(ctx, "AUTH %s", tmp.c_str()));
		if (!reply)
			throw DatabaseException("Redis authentication failed");
		if (reply->type == REDIS_REPLY_ERROR) {
			std::string err =
					"Redis authentication failed: " + std::string(reply->str, reply->len);
...
void Database_Redis::beginSave()
{
	...
	if (!reply) {
		throw DatabaseException(
				std::string("Redis command 'MULTI' failed: ") + ctx->errstr);
	}
	freeReplyObject(reply);
}
...
void Database_Redis::endSave()
{
	...
	if (!reply) {
		throw DatabaseException(
				std::string("Redis command 'EXEC' failed: ") + ctx->errstr);
	}
	freeReplyObject(reply);
}
...
bool Database_Redis::saveBlock(const v3s16 &pos, const std::string &data)
{
	...
	redisReply *reply = static_cast<redisReply *>(redisCommand(
			ctx, "HSET %s %s %b", hash.c_str(), tmp.c_str(), data.c_str(), data.size()));
	if (!reply) {
...
void Database_Redis::loadBlock(const v3s16 &pos, std::string *block)
{
	...
	if (!reply) {
		throw DatabaseException(
				std::string("Redis command 'HGET %s %s' failed: ") + ctx->errstr);
	}
	...
	case REDIS_REPLY_ERROR: {
		...
		freeReplyObject(reply);
		errorstream << "loadBlock: loading block " << PP(pos) << " failed: " << errstr
					<< std::endl;
		throw DatabaseException(
				std::string("Redis command 'HGET %s %s' errored: ") + errstr);
	}
...
bool Database_Redis::deleteBlock(const v3s16 &pos)
{
	...
	if (!reply) {
		throw DatabaseException(
				std::string("Redis command 'HDEL %s %s' failed: ") + ctx->errstr);
...
{
	redisReply *reply =
			static_cast<redisReply *>(redisCommand(ctx, "HKEYS %s", hash.c_str()));
	if (!reply) {
		throw DatabaseException(
				std::string("Redis command 'HKEYS %s' failed: ") + ctx->errstr);
	}
	switch (reply->type) {
	case REDIS_REPLY_ARRAY:
		...
		break;
	case REDIS_REPLY_ERROR:
		throw DatabaseException(std::string("Failed to get keys from database: ") +
