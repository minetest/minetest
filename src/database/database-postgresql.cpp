...
void Database_PostgreSQL::connectToDatabase()
{
	...
	if (PQstatus(m_conn) != CONNECTION_OK) {
		throw DatabaseException(
				std::string("PostgreSQL database error: ") + PQerrorMessage(m_conn));
	}
	...
	*/
	if (m_pgversion < 90500) {
		warningstream << "Your PostgreSQL server lacks UPSERT "
					  << "support. Use version 9.5 or better if possible." << std::endl;
	}

	infostream << "PostgreSQL Database: Version " << m_pgversion << " Connection made."
			   << std::endl;
...
{
	std::string sql_check_table =
			"SELECT relname FROM pg_class WHERE relname='" + table_name + "';";
...
void MapDatabasePostgreSQL::loadBlock(const v3s16 &pos, std::string *block)
{
	...
	PGresult *results =
			execPrepared("read_block", ARRLEN(args), args, argLen, argFmt, false);
...
void MapDatabasePostgreSQL::listAllLoadableBlocks(std::vector<v3s16> &dst)
{
	...
	PGresult *results =
			execPrepared("list_all_loadable_blocks", 0, NULL, NULL, NULL, false, false);
...
void PlayerDatabasePostgreSQL::initStatements()
{
	...
	prepareStatement(
			"remove_player_metadata", "DELETE FROM player_metadata WHERE player = $1");
...
void PlayerDatabasePostgreSQL::savePlayer(RemotePlayer *player)
{
	...
	const char *values[] = { player->getName(), pitch.c_str(), yaw.c_str(), posx.c_str(),
		posy.c_str(), posz.c_str(), hp.c_str(), breath.c_str() };
	...
	for (u16 i = 0; i < inventory_lists.size(); i++) {
		...
		const char *inv_values[] = { player->getName(), inv_id.c_str(), width.c_str(),
			name.c_str(), lsize.c_str() };
...
bool PlayerDatabasePostgreSQL::loadPlayer(RemotePlayer *player, PlayerSAO *sao)
{
	...
	for (int row = 0; row < resultCount; ++row) {
		...
		PGresult *results2 =
				execPrepared("load_player_inventory_items", 2, values2, false, false);
...
void AuthDatabasePostgreSQL::initStatements()
{
	...
	prepareStatement(
			"auth_read_privs", "SELECT privilege FROM user_privileges WHERE id = $1");
...
void AuthDatabasePostgreSQL::listNames(std::vector<std::string> &res)
{
	...
	PGresult *results =
			execPrepared("auth_list_names", 0, NULL, NULL, NULL, false, false);
