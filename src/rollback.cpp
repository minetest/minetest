		...
		const std::list<ActionRow> & rows)
{
	...
	for (const ActionRow &row : rows) {
		...
		case RollbackAction::TYPE_SET_NODE:
			action.p            = v3POS(row.x, row.y, row.z);
...
float RollbackManager::getSuspectNearness(bool is_guess, v3POS suspect_p,
...
std::string RollbackManager::getSuspect(v3POS p, float nearness_shortcut,
		float min_nearness)
{
...
std::list<RollbackAction> RollbackManager::getNodeActors(v3POS pos, int range,
