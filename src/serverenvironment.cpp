...
void LBMManager::applyLBMs(ServerEnvironment *env, MapBlock *block, u32 stamp)
{
	...
	FATAL_ERROR_IF(!m_query_mode,
		"attempted to query on non fully set up LBMManager");
	v3POS pos_of_block = block->getPosRelative();
	v3POS pos;
	MapNode n;
	content_t c;
...
void fillRadiusBlock(v3POS p0, s16 r, std::set<v3POS> &list)
{
	v3POS p;
...
void fillViewConeBlock(v3POS p0,
	const s16 r,
...
void ActiveBlockList::update(std::vector<PlayerSAO*> &active_players,
	...
	std::set<v3BPOS> &blocks_removed,
	std::set<v3BPOS> &blocks_added)
{
	/*
		Create the new list
	*/
	std::set<v3POS> newlist = m_forceloaded_list;
	m_abm_list = m_forceloaded_list;
	for (const PlayerSAO *playersao : active_players) {
		v3BPOS pos = getNodeBlockPos(floatToInt(playersao->getBasePosition(), BS));
...
public:
	...
	void apply(MapBlock *block, int &blocks_scanned, int &abms_run, int &blocks_cached)
	{
		...
		for(p0.Z=0; p0.Z<MAP_BLOCKSIZE; p0.Z++)
		{
			...
			v3POS p = p0 + block->getPosRelative();
...
void ServerEnvironment::activateBlock(MapBlock *block, u32 additional_dtime)
{
	...
	if (!elapsed_timers.empty()) {
		...
		for (const NodeTimer &elapsed_timer : elapsed_timers) {
			...
			v3POS p = elapsed_timer.position + block->getPosRelative();
...
bool ServerEnvironment::setNode(v3POS p, const MapNode &n)
{
...
bool ServerEnvironment::removeNode(v3POS p)
{
	const NodeDefManager *ndef = m_server->ndef();
	MapNode n_old = m_map->getNode(p);
...
bool ServerEnvironment::swapNode(v3POS p, const MapNode &n)
{
...
u8 ServerEnvironment::findSunlight(v3POS pos) const
{
	...
	static const v3POS dirs[] = {
		v3POS(-1, 0, 0), v3POS(1, 0, 0), v3POS(0, 0, -1), v3POS(0, 0, 1),
		v3POS(0, -1, 0), v3POS(0, 1, 0)
	};
	...
	while (!stack.empty()) {
		...
		v3POS currentPos = e.pos;
		s8 dist = e.dist + 1;

		for (const v3POS& off : dirs) {
			v3POS neighborPos = currentPos + off;
...
void ServerEnvironment::clearObjects(ClearObjectsMode mode)
{
	...
	std::vector<v3POS> loaded_blocks;
	infostream << "ServerEnvironment::clearObjects(): "
	...
	std::vector<v3POS> loadable_blocks;
...
void ServerEnvironment::step(float dtime)
{
	...
	*/
	if (m_active_blocks_management_interval.step(dtime, m_cache_active_block_mgmt_interval)) {
		...
		*/
		...
		for (const v3POS &p: blocks_removed) {
			MapBlock *block = m_map->getBlockNoCreateNoEx(p);
			if (!block)
				continue;
		...
		*/
		...
		for (const v3POS &p: blocks_added) {
	...
	*/
	if (m_active_blocks_nodemetadata_interval.step(dtime, m_cache_nodetimer_interval)) {
		...
		for (const v3POS &p: m_active_blocks.m_list) {
			MapBlock *block = m_map->getBlockNoCreateNoEx(p);
			if (!block)
				continue;
	...
	if (m_active_block_modifier_interval.step(dtime, m_cache_abm_interval)) {
		...
		std::vector<v3POS> output(m_active_blocks.m_abm_list.size());
...
ServerEnvironment::BlockStatus ServerEnvironment::getBlockStatus(v3BPOS blockpos)
{
...
void ServerEnvironment::setStaticForActiveObjectsInBlock(
	v3POS blockpos, bool static_exists, v3POS static_block)
{
	...
	bool set_changed, u32 dtime_s)
{
	...
	if (object->isStaticAllowed()) {
		...
		v3BPOS blockpos = getNodeBlockPos(floatToInt(objectpos, BS));
		...
		if(block){
			...
			v3POS p = floatToInt(objectpos, BS);
...
void ServerEnvironment::deactivateFarObjects(bool _force_delete)
{
	auto cb_deactivate = [this, _force_delete] (ServerActiveObject *obj, u16 id) {
		...
		v3BPOS blockpos_o = getNodeBlockPos(floatToInt(objectpos, BS));
