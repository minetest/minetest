...
irr::video::CPostProcessingEffectChain::CPostProcessingEffectChain(
		...
		irr::video::E_POSTPROCESSING_EFFECT_QUALITY defQuality,
...
irr::video::CPostProcessingEffect* irr::video::CPostProcessingEffectChain::createEffect(irr::video::E_POSTPROCESSING_EFFECT type)
{
	...
	switch(type)
	{
		...
		case EPE_MUL2:
			shaderSource = readShader("mul2.frag");
			effectName = "mul2";
			break;
		...
		case EPE_CEL:
			shaderSource = readShader("cel.frag");
			effectName = "cel";
			break;
		case EPE_UC2_TONEMAP:
			shaderSource = readShader("uc2_tonemap.frag");
			effectName = "uc2_tonemap";
			break;
...
void irr::video::CPostProcessingEffectChain::detachEffect(
...
void irr::video::CPostProcessingEffectChain::removeEffect(
		irr::video::CPostProcessingEffect* effect)
{
	detachEffect(effect);
	delete effect;
...
void irr::video::CPostProcessingEffectChain::setKeepOriginalRender(bool keep)
{
	...
	if (keep && !getKeepOriginalRender()) {
		...
		OriginalRender = video->addRenderTargetTexture(
			video->getCurrentRenderTargetSize(), originalName.c_str());
	}
