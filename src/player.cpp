...
// Temporary option for old move code
void Player::old_move(f32 dtime, Environment *env, f32 pos_max_d,
		std::vector<CollisionInfo> *collision_info)
{
	Map *map = &env->getMap();
	const NodeDefManager *nodemgr = getNodeDefManager();
	...
	bool noclip = checkPrivilege("noclip") &&
		g_settings->getBool("noclip");
	bool free_move = noclip && g_settings->getBool("free_move") && fly_allowed;
	if (free_move) {
		position += m_speed * dtime;
		setPosition(position);
		m_sneak_node_exists = false;
...
//endif
	...
	*/
	const ContentFeatures &f = nodemgr->get(map->getNodeNoEx(
		m_sneak_node_exists ? m_sneak_node : m_standing_node
	));
	// Determine if jumping is possible
