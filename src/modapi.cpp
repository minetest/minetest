...
bool ModApiBasic::Initialize(lua_State* L,int top) {
	...
	retval &= API_FCT(register_biome_groups);
...
int ModApiBasic::l_register_biome_groups(lua_State *L)
{
	luaL_checktype(L, 1, LUA_TTABLE);
	int index = 1;

	BiomeDefManager *bmgr = getServer(L)->getBiomeDef();
	if (!bmgr) {
...
int ModApiBasic::l_register_biome(lua_State *L)
{
	...
	IWritableNodeDefManager *ndef = getServer(L)->getWritableNodeDefManager();
	BiomeDefManager *bmgr = getServer(L)->getBiomeDef();
	if (!bmgr) {
