...
#define AST_CONTAINS_AREA(amine, amaxe, b)                                               \
	(AST_SMALLER_EQ_AS((amine), (b)->minedge) && AST_SMALLER_EQ_AS((b)->maxedge, (amaxe)))
...
bool VectorAreaStore::insertArea(Area *a)
{
	if (a->id == U32_MAX)
		a->id = getNextId();
	std::pair<AreaMap::iterator, bool> res = areas_map.insert(std::make_pair(a->id, *a));
	if (!res.second)
		// ID is not unique
		return false;
...
void VectorAreaStore::getAreasInArea(
		std::vector<Area *> *result, v3s16 minedge, v3s16 maxedge, bool accept_overlap)
{
	for (Area *area : m_areas) {
...
void SpatialAreaStore::getAreasInArea(
		std::vector<Area *> *result, v3s16 minedge, v3s16 maxedge, bool accept_overlap)
{
	VectorResultVisitor visitor(result, this);
	if (accept_overlap) {
