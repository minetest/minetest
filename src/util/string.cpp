...
std::string wide_to_utf8(const std::wstring &input)
{
	...
	memset(outbuf, 0, outbuf_size);
	WideCharToMultiByte(
			CP_UTF8, 0, input.c_str(), input.size(), outbuf, outbuf_size, NULL, NULL);
	std::string out(outbuf);
	delete[] outbuf;
	return out;
}
...
std::string urlencode(const std::string &str)
{
	...
	for (unsigned char c : str) {
		if (isalnum(c) || c == '-' || c == '.' || c == '_' || c == '~') {
			...
			oss << "%" << url_hex_chars[(c & 0xf0) >> 4] << url_hex_chars[c & 0x0f];
		}
	}
	return oss.str();
}
...
static bool parseHexColorString(
		const std::string &value, video::SColor &color, unsigned char default_alpha)
{
...
void translate_all(
		const std::wstring &s, size_t &i, Translations *translations, std::wstring &res);
		...
		const std::wstring &textdomain, size_t &i, std::wstring &res)
{
	...
	while (i < s.length()) {
		...
		if (parts[0] == L"E") {
			...
			errorstream << "Ignoring escape sequence '" << wide_to_narrow(escape_sequence)
	...
	while (j < toutput.length()) {
		...
		if (0 <= arg_index && (size_t)arg_index < args.size()) {
			...
			errorstream
					<< "Ignoring out-of-bounds argument escape sequence in translation"
					<< std::endl;
		}
	}
	res = result.str();
