 ...
 */
const static v3POS neighbor_dirs[6] = {
	v3POS(1, 0, 0), // right
	v3POS(0, 1, 0), // top
	v3POS(0, 0, 1), // back
	v3POS(0, 0, -1), // front
...
struct SunlightPropagationUnit{
	...
	SunlightPropagationUnit(v2POS relpos, bool sunlit):
		relative_pos(relpos),
		is_sunlit(sunlit)
	{}
};
 ...
 */
bool is_sunlight_above(Map *map, v3POS pos, const NodeDefManager *ndef)
{
	bool sunlight = true;
	mapblock_v3 source_block_pos;
	relative_v3 source_rel_pos;
	getNodeBlockPosWithOffset(pos + v3POS(0, 1, 0), source_block_pos,
		source_rel_pos);
		...
		for (auto it = oldnodes.cbegin(); it < oldnodes.cend(); ++it) {
			...
			if (ndef->get(n).light_propagates) {
				if (bank == LIGHTBANK_DAY && ndef->get(n).sunlight_propagates
					...
					for (const v3POS &neighbor_dir : neighbor_dirs) {
						v3POS p2 = p + neighbor_dir;
						bool is_valid;
...
const VoxelArea block_borders[] = {
	VoxelArea(v3POS(15, 0, 0), v3POS(15, 15, 15)), //X+
	VoxelArea(v3POS(0, 15, 0), v3POS(15, 15, 15)), //Y+
	VoxelArea(v3POS(0, 0, 15), v3POS(15, 15, 15)), //Z+
	VoxelArea(v3POS(0, 0, 0), v3POS(15, 15, 0)),   //Z-
	VoxelArea(v3POS(0, 0, 0), v3POS(15, 0, 15)),   //Y-
	VoxelArea(v3POS(0, 0, 0), v3POS(0, 15, 15))    //X-
};
	...
	std::map<v3POS, MapBlock*> &modified_blocks)
{
	...
	for (LightBank bank : banks) {
		...
		for (direction d = 0; d < 6; d++) {
			// For each direction
			// Get neighbor block
			v3POS otherpos = block->getPos() + neighbor_dirs[d];
 ...
 */
void fill_with_sunlight(MMVManip *vm, const NodeDefManager *ndef, v2POS offset,
	bool light[MAP_BLOCKSIZE][MAP_BLOCKSIZE])
{
	...
	for (s16 x = 0; x < MAP_BLOCKSIZE; x++) {
		...
		v2POS realpos = offset + v2POS(x, z);
...
{
	mapblock_v3 source_block_pos = pos + v3POS(0, 1, 0);
...
const VoxelArea block_pad[] = {
	VoxelArea(v3POS(15, 0, 0), v3POS(15, 15, 15)), //X+
	VoxelArea(v3POS(1, 15, 0), v3POS(14, 15, 15)), //Y+
	VoxelArea(v3POS(1, 1, 15), v3POS(14, 14, 15)), //Z+
	VoxelArea(v3POS(1, 1, 0), v3POS(14, 14, 0)),   //Z-
	VoxelArea(v3POS(1, 0, 0), v3POS(14, 0, 15)),   //Y-
	VoxelArea(v3POS(0, 0, 0), v3POS(0, 15, 15))    //X-
};
	...
	std::map<v3POS, MapBlock*> *modified_blocks)
{
	...
	// For each map block:
	for (BPOS x = minblock.X; x <= maxblock.X; x++)
	for (BPOS z = minblock.Z; z <= maxblock.Z; z++) {
		// Extract sunlight above.
		is_sunlight_above_block(map, v3BPOS(x, maxblock.Y, z), ndef, lights);
		v2POS offset(x, z);
		offset *= MAP_BLOCKSIZE;
		// Reset the voxel manipulator.
		fill_with_sunlight(vm, ndef, offset, lights);
		// Copy sunlight data
		data.target_block = v3POS(x, minblock.Y - 1, z);
		for (s16 z = 0; z < MAP_BLOCKSIZE; z++)
		for (s16 x = 0; x < MAP_BLOCKSIZE; x++)
			data.data.emplace_back(v2POS(x, z), lights[z][x]);
	...
	for (blockpos.Z = minblock.Z; blockpos.Z <= maxblock.Z; blockpos.Z++) {
		...
		if (!block || block->isDummy())
			// Skip not existing blocks.
			continue;
		v3POS offset = block->getPosRelative();
	...
	std::map<v3POS, MapBlock*> *modified_blocks)
{
	...
	// Copy sunlight data
	data.target_block = v3POS(blockpos.X, blockpos.Y - 1, blockpos.Z);
	for (s16 z = 0; z < MAP_BLOCKSIZE; z++)
	for (s16 x = 0; x < MAP_BLOCKSIZE; x++) {
		data.data.emplace_back(v2POS(x, z), lights[z][x]);
	}
...
POS VoxelLineIterator::getIndex(v3POS voxel){
	return
