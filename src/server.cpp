	...
	m_ignore_map_edit_events_peer_id(0)
{
	...
	m_ScriptIface = new ScriptApi(this);


	// Load and run builtin.lua
	infostream<<"Server: Loading builtin.lua [\""
			<<builtinpath<<"\"]"<<std::endl;
	bool success = m_ScriptIface->loadMod(builtinpath, "__builtin");
	if(!success){
	...
	for(std::vector<ModSpec>::iterator i = m_mods.begin();
		...
		infostream<<"  ["<<padStringRight(mod.name, 12)<<"] [\""
				<<scriptpath<<"\"]"<<std::endl;
		bool success = m_ScriptIface->loadMod(scriptpath, mod.name);
		if(!success){
			errorstream<<"Server: Failed to load and run "
					<<scriptpath<<std::endl;
	...
	m_env = new ServerEnvironment(servermap, m_ScriptIface, this, this);
	
	m_emerge->initMapgens(servermap->getMapgenParams());

	// Give environment reference to scripting api
	m_ScriptIface->initializeEnvironment(m_env);
...
void Server::ProcessData(u8 *data, u32 datasize, u16 peer_id)
{
	...
	if(command == TOSERVER_INIT)
	{
		...
		*/
		...
		bool has_auth = m_ScriptIface->getAuth(playername, &checkpwd, NULL);
		...
		if(!has_auth){
			...
			m_ScriptIface->createAuth(playername, initial_password);
		}

		has_auth = m_ScriptIface->getAuth(playername, &checkpwd, NULL);
	...
	else if(command == TOSERVER_CHAT_MESSAGE)
	{
		...
		*/
		...
		// Run script hook
		bool ate = m_ScriptIface->on_chat_message(player->getName(),
	...
	else if(command == TOSERVER_PASSWORD)
	{
		...
		*/
		...
		std::string checkpwd;
		m_ScriptIface->getAuth(playername, &checkpwd, NULL);
		...
		bool success = m_ScriptIface->setPassword(playername, newpwd);
		if(success){
	...
	else if(command == TOSERVER_INTERACT)
	{
			...
			if(pointed.type == POINTEDTHING_NODE)
			{
				...
				*/
				...
				if(n.getContent() != CONTENT_IGNORE)
					m_ScriptIface->node_on_punch(p_under, n, playersao);
				// Cheat prevention
		...
		else if(action == 2)
		{
			...
			if(pointed.type == POINTEDTHING_NODE)
			{
				...
				if(is_valid_dig && n.getContent() != CONTENT_IGNORE)
					m_ScriptIface->node_on_dig(p_under, n, playersao);
		...
		*/
		else if(action == 3)
		{
			...
			else if(m_ScriptIface->item_OnPlace(
	...
	else if(command == TOSERVER_NODEMETA_FIELDS)
	{
		...
		m_ScriptIface->node_on_receive_fields(p, formname, fields,playersao);
	...
	else if(command == TOSERVER_INVENTORY_FIELDS)
	{
		...
		m_ScriptIface->on_playerReceiveFields(playersao, formname, fields);
	}
	else
	{
...
void Server::SendPlayerPrivileges(u16 peer_id)
{
	...
	m_ScriptIface->getAuth(player->getName(), NULL, &privs);
...
void Server::DiePlayer(u16 peer_id)
{
	...
	m_ScriptIface->on_dieplayer(playersao);
...
void Server::RespawnPlayer(u16 peer_id)
{
	...
	bool repositioned = m_ScriptIface->on_respawnplayer(playersao);
	if(!repositioned){
...
{
	std::set<std::string> privs;
	m_ScriptIface->getAuth(name, NULL, &privs);
	return privs;
}
...
PlayerSAO* Server::emergePlayer(const char *name, u16 peer_id)
{
	...
	*/
	...
	/* Run scripts */
	if(newplayer)
		m_ScriptIface->on_newplayer(playersao);

	m_ScriptIface->on_joinplayer(playersao);
		...
		*/
		...
		/* Run scripts and remove from environment */
		{
			if(player != NULL)
			{
				...
				m_ScriptIface->on_leaveplayer(playersao);
