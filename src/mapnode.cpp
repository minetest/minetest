...
static const Rotation wallmounted_to_rot[] = { ROTATE_0, ROTATE_180, ROTATE_90,
	ROTATE_270 };
...
{
	const ContentFeatures &f = nodemgr->get(*this);
	if (f.param_type_2 == CPT2_WALLMOUNTED || f.param_type_2 == CPT2_COLORED_WALLMOUNTED)
...
void transformNodeBox(const MapNode &n, const NodeBox &nodebox,
		const NodeDefManager *nodemgr, std::vector<aabb3f> *p_boxes, u8 neighbors = 0)
{
	...
	if (nodebox.type == NODEBOX_FIXED || nodebox.type == NODEBOX_LEVELED) {
		...
		// side
			...
			v3f vertices[2] = { nodebox.wall_side.MinEdge, nodebox.wall_side.MaxEdge };
...
#define BOXESPUSHBACK(c)                                                                 \
	for (std::vector<aabb3f>::const_iterator it = (c).begin(); it != (c).end(); ++it)    \
		(boxes).push_back(*it);
...
static inline void getNeighborConnectingFace(const v3s16 &p,
		const NodeDefManager *nodedef, Map *map, MapNode n, u8 bitmask, u8 *neighbors)
{
...
void MapNode::getNodeBoxes(
		const NodeDefManager *nodemgr, std::vector<aabb3f> *boxes, u8 neighbors) const
{
	const ContentFeatures &f = nodemgr->get(*this);
	transformNodeBox(*this, f.node_box, nodemgr, boxes, neighbors);
}

void MapNode::getCollisionBoxes(
		const NodeDefManager *nodemgr, std::vector<aabb3f> *boxes, u8 neighbors) const
{
	const ContentFeatures &f = nodemgr->get(*this);
...
void MapNode::getSelectionBoxes(
		const NodeDefManager *nodemgr, std::vector<aabb3f> *boxes, u8 neighbors) const
{
	const ContentFeatures &f = nodemgr->get(*this);
		...
		u32 nodecount, u8 content_width, u8 params_width, bool compressed)
{
	...
	if (version < 22 || (content_width != 1 && content_width != 2) || params_width != 2)
