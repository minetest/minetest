...
MapgenV5::MapgenV5(int mapgenid, MapgenParams *params, EmergeManager *emerge_) {
	...
	this->zstride = csize.X * (csize.Y + 1);
	...
	noise_ground       = new Noise(&sp->np_ground,  seed, csize.X, csize.Y + 1, csize.Z);
	noise_crumble      = new Noise(&sp->np_crumble, seed, csize.X, csize.Y + 1, csize.Z);
	noise_wetness      = new Noise(&sp->np_wetness, seed, csize.X, csize.Y + 1, csize.Z);
...
void MapgenV5::generateBaseTerrain() {
	...
	for(s16 z=node_min.Z; z<=node_max.Z; z++) {
		for(s16 y=node_min.Y; y<=node_max.Y + 1; y++) {
...
void MapgenV5::generateBiomes() {
	...
	for (s16 x = node_min.X; x <= node_max.X; x++, index++) {
		...
		for (s16 y = node_max.Y + 1; y >= node_min.Y; y--) {
			content_t c = vm->m_data[i].getContent();
