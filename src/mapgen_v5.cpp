...
MapgenV5::MapgenV5(int mapgenid, MapgenParams *params, EmergeManager *emerge_) {
	...
	this->zstride = csize.X * (csize.Y + 4);
	...
	// 3D terrain noise
	noise_cave1        = new Noise(&sp->np_cave1,   seed, csize.X, csize.Y + 4, csize.Z);
	noise_cave2        = new Noise(&sp->np_cave2,   seed, csize.X, csize.Y + 4, csize.Z);
	noise_ground       = new Noise(&sp->np_ground,  seed, csize.X, csize.Y + 4, csize.Z);
	noise_crumble      = new Noise(&sp->np_crumble, seed, csize.X, csize.Y + 4, csize.Z);
	noise_wetness      = new Noise(&sp->np_wetness, seed, csize.X, csize.Y + 4, csize.Z);
	...
	for (s16 x = node_min.X; x <= node_max.X; x++, index++) {
		...
		for (s16 y = node_max.Y + 1; y >= node_min.Y - 3; y--) {
			...
			if (is_replaceable_content && have_air) {
				...
				if (c_below != CONTENT_AIR && c_below != CONTENT_IGNORE) {
					if (nplaced < y0_top) {
