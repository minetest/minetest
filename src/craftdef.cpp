...
static bool inputItemMatchesRecipe(
		const std::string &inp_name, const std::string &rec_name, IItemDefManager *idef)
{
	// Exact name
	...
	do {
		...
		for (size_t i = 0; i < recipe.size(); i++) {
			...
			if (!inputItemMatchesRecipe(
						input_filtered[i], recipe_copy[i], gamedef->idef())) {
				all_match = false;
				break;
			}
		}
...
public:
			...
			IGameDef *gamedef) const
	{
		...
		for (int type = 0; type <= craft_hash_type_max; type++) {
			...
			for (std::vector<CraftDefinition *>::size_type i = hash_collisions.size();
					i > 0; i--) {
				...
				if (priority > priority_best && def->check(input, gamedef)) {
					...
					if (!is.isKnown(gamedef->idef())) {
						infostream << "trying to craft non-existent " << out.item
								   << ", ignoring recipe" << std::endl;
						continue;
					}
			...
			CraftOutput &output, IGameDef *gamedef, unsigned limit = 0) const
	{
		...
		for (std::vector<CraftDefinition *>::size_type i = vec.size(); i > 0; i--) {
	...
	virtual bool clearCraftsByInput(const CraftInput &input, IGameDef *gamedef)
	{
		...
		for (auto def : defs) {
			...
			outdefs.erase(
					std::remove(outdefs.begin(), outdefs.end(), def), outdefs.end());
		}
		if (got_hit)
			defs.swap(new_defs);
	...
	virtual std::string dump() const
	{
		...
		for (int type = 0; type <= craft_hash_type_max; ++type) {
			for (auto it = m_craft_defs[type].begin(); it != m_craft_defs[type].end();
					++it) {
				for (std::vector<CraftDefinition *>::size_type i = 0;
						i < it->second.size(); i++) {
					os << "type " << type << " hash " << it->first << " def "
					   << it->second[i]->dump() << "\n";
				}
			}
		}
	...
	virtual void registerCraft(CraftDefinition *def, IGameDef *gamedef)
	{
		...
		std::string output_name =
				craftGetItemName(def->getOutput(input, gamedef).item, gamedef);
