// Copyright (C) 2002-2012 Nikolaus Gebhardt (Modified by Mustapha T.)
...
GUIEditBoxWithScrollBar::GUIEditBoxWithScrollBar(const wchar_t* text, bool border,
	...
	const core::rect<s32>& rectangle, u32 scrollbarwidth)
	...
	m_scrollbar_width(scrollbarwidth), m_vscrollbar(NULL)
{
#ifdef _DEBUG
...
void GUIEditBoxWithScrollBar::draw()
{
	...
	if (m_border) {
		// draw the border
		skin->draw3DSunkenPane(this, skin->getColor(bgCol), false, m_background, AbsoluteRect, &AbsoluteClippingRect);
...
{
	irr::core::rect<s32> scrollbarrect(m_frame_rect.UpperLeftCorner + irr::core::position2di(m_frame_rect.getWidth() - m_scrollbar_width, 0), m_frame_rect.LowerRightCorner);
