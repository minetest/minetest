...
inline double decode_light_f(double light_f)
{
	s32 i = (u32)(light_f * LIGHT_MAX + 0.5);

	if(i <= 0)
		return (light_decode_table[0] / 255.0);
	if(i >= LIGHT_MAX)
		return (light_decode_table[LIGHT_MAX] / 255.0);

	double v1 = light_decode_table[i-1] / 255.0;
	double v2 = light_decode_table[i] / 255.0;
	double f0 = i - 0.5;
	double f = light_f * LIGHT_MAX - f0;
...
// 0 <= lightday, lightnight <= LIGHT_SUN
// 0 <= return value <= 255
inline u8 blend_light_f1(double daylight_factor, u8 lightday, u8 lightnight)
{
