...
unsigned long httpfetch_caller_alloc()
{
	...
	for (unsigned long caller = discard + 1; caller != discard; ++caller) {
		...
		if (it == g_httpfetch_results.end()) {
			verbosestream << "httpfetch_caller_alloc: allocating " << caller << std::endl;
...
unsigned long httpfetch_caller_alloc_secure()
{
	...
	verbosestream << "httpfetch_caller_alloc_secure: allocating " << caller << std::endl;
...
public:
	...
	~CurlHandlePool()
	{
		for (std::list<CURL *>::iterator it = handles.begin(); it != handles.end();
				++it) {
	...
	if (request.post_fields.empty() && request.post_data.empty()) {
		...
		for (StringMap::iterator it = request.post_fields.begin();
			...
			curl_formadd(&post, &last, CURLFORM_NAMELENGTH, it->first.size(),
					CURLFORM_PTRNAME, it->first.c_str(), CURLFORM_CONTENTSLENGTH,
					it->second.size(), CURLFORM_PTRCONTENTS, it->second.c_str(),
					CURLFORM_END);
		}
...
const HTTPFetchResult *HTTPFetchOngoing::complete(CURLcode res)
{
	...
	if (res != CURLE_OK) {
		errorstream << request.url << " not found (" << curl_easy_strerror(res) << ")"
					<< " (response code " << result.response_code << ")" << std::endl;
	}
...
protected:
	...
	void processRequest(const Request &req)
	{
		if (req.type == RT_FETCH) {
			...
			for (std::vector<HTTPFetchOngoing *>::iterator it = m_all_ongoing.begin();
			...
			for (std::list<HTTPFetchRequest>::iterator it = m_queued_fetches.begin();
	...
	{
		while (m_all_ongoing.size() < m_parallel_limit && !m_queued_fetches.empty()) {
	...
	void waitForIO(long timeout)
	{
		...
		mres = curl_multi_fdset(
				m_multi, &read_fd_set, &write_fd_set, &exc_fd_set, &max_fd);
		if (mres != CURLM_OK) {
