...
void sendAnnounce(std::string action, std::vector<std::string> m_clients_names, double uptime, u32 game_time, std::string gameid, std::vector<ModSpec> m_mods) {
	Json::Value server;
	if (action.size())
	...
	if (action != "delete") {
		...
		server["clients"]	= (int)m_clients_names.size();
		server["clients_max"]	= g_settings->get("max_users");
		server["clients_list"]	= Json::Value(Json::arrayValue);
		for(u32 i = 0; i < m_clients_names.size(); ++i) {
			server["clients_list"].append(m_clients_names[i]);
		}
