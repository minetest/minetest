...
// Returns common path from two branching paths, difference stored in last argument
std::string GetCommonPath(std::string path1, std::string path2, int &diff);

// Returns relative path generated by comparing path1 & path2; if path1 is empty or
// calculated parents count is too high it returns path2 instead
// The paths must be absolute, max_parents is maximum amount of parent markers allowed
