...
void MapgenValleys::generateCaves(s16 max_stone_y, s16 large_cave_depth)
{
	...
	float yblmin = -map_generation_limit + massive_cave_blend * 1.5f;
	...
	s16 lava_depth = ceil((lava_max_height - node_min.Y + 1) * 10.f / map_generation_limit);
	...
	s16 water_depth = ceil((map_generation_limit - abs(node_min.Y) + 1) * 10.f / map_generation_limit);
