...
void main() {
	...
	vec3 rgbNW = texture2D(Render, uv + (vec2(-1.0, -1.0) * PixelSize)).xyz;
	vec3 rgbNE = texture2D(Render, uv + (vec2(+1.0, -1.0) * PixelSize)).xyz;
	vec3 rgbSW = texture2D(Render, uv + (vec2(-1.0, +1.0) * PixelSize)).xyz;
	vec3 rgbSE = texture2D(Render, uv + (vec2(+1.0, +1.0) * PixelSize)).xyz;
	...
	dir = min(vec2(FXAA_SPAN_MAX,	FXAA_SPAN_MAX),
				max(vec2(-FXAA_SPAN_MAX, -FXAA_SPAN_MAX), dir * rcpDirMin)) * PixelSize	;
